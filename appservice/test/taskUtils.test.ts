...
suite("taskUtils", () => {
	test("getFsPathFromTask", () => {
		assert.equal(
			taskUtils.getFsPathFromTask(tasks.js.install),
			"/Users/erijiz/TestRepos/js1"
		);
		assert.equal(
			taskUtils.getFsPathFromTask(tasks.other.global),
			undefined
		);
		assert.equal(
			taskUtils.getFsPathFromTask(tasks.other.workspace),
			undefined
		);
		assert.equal(
			taskUtils.getFsPathFromTask(tasks.other.actualGlobal),
			undefined
		);
		assert.equal(
			taskUtils.getFsPathFromTask(tasks.other.noScope),
			undefined
		);
	});
	...
	suite("isTaskInScopeOfPath", () => {
		test("true", () => {
			...
			assert.equal(
				taskUtils.isTaskInScopeOfPath(
					tasks.js.install,
					"/Users/erijiz/TestRepos/js1/subfolder"
				),
				true
			);
			assert.equal(
				taskUtils.isTaskInScopeOfPath(
					tasks.js.install,
					"/Users/erijiz/TestRepos/js1/subfolder/sub2"
				),
				true
			);
			assert.equal(
				taskUtils.isTaskInScopeOfPath(
					tasks.js.install,
					"/Users/erijiz/TestRepos/js1/../js1/"
				),
				true
			);
		});
	...
	suite("isTaskScopeEqual", () => {
		test("true", () => {
			assert.equal(
				taskUtils.isTaskScopeEqual(tasks.js.install, tasks.js.install),
				true
			);
			assert.equal(
				taskUtils.isTaskScopeEqual(tasks.js.install, tasks.js.start),
				true
			);
			assert.equal(
				taskUtils.isTaskScopeEqual(tasks.js.install, tasks.js.prune),
				true
			);
		});
		...
		test("false", () => {
			assert.equal(
				taskUtils.isTaskScopeEqual(tasks.js.install, tasks.ts.install),
				false
			);
			assert.equal(
				taskUtils.isTaskScopeEqual(tasks.js.install, tasks.ts.start),
				false
			);
			assert.equal(
				taskUtils.isTaskScopeEqual(tasks.js.install, tasks.cs.build),
				false
			);
			assert.equal(
				taskUtils.isTaskScopeEqual(
					tasks.js.install,
					tasks.other.global
				),
				false
	...
	suite("isTaskEqual", () => {
		test("true, shell task", () => {
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.install, tasks.js.install),
				true
			);
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.install, {
					...tasks.js.install,
				}),
				true
			);
		...
		test("true, func task", () => {
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.start, tasks.js.start),
				true
			);
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.start, { ...tasks.js.start }),
		...
		test("false", () => {
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.install, tasks.js.start),
				false
			);
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.install, tasks.js.prune),
				false
			);
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.install, tasks.ts.install),
				false
			);
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.install, tasks.ts.start),
				false
			);
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.install, tasks.cs.build),
				false
			);
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.install, tasks.other.global),
				false
			);
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.install, tasks.other.workspace),
				false
			);
			assert.equal(
				taskUtils.isTaskEqual(
					tasks.js.install,
					tasks.other.actualGlobal
				),
				false
			...
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.install, tasks.other.noScope),
				false
			);
			assert.equal(
				taskUtils.isTaskEqual(tasks.js.start, tasks.other.nonFuncType),
				false
			);
		});
	});
	...
	suite("findTask", () => {
		async function testFindTask(
			taskPath: string,
			taskName: string,
			expectedTask: Task | undefined
		): Promise<void> {
			assert.equal(
				await taskUtils.findTask(taskPath, taskName, allTasks),
				expectedTask
			);
			// test against reversed list just to make sure it didn't get lucky based on the order it searched
			assert.equal(
				await taskUtils.findTask(taskPath, taskName, allTasksReversed),
				expectedTask
			);
		}
		...
		test("other", async () => {
			...
			await testFindTask(
				"/Users/erijiz/TestRepos/js1",
				"notfunc: host start",
				tasks.other.nonFuncType
			);
			await testFindTask(
				"/Users/erijiz/TestRepos/js1",
				"tsc: build - tsconfig.json",
				tasks.other.tscBuild
			);
			await testFindTask(
				"/Users/erijiz/TestRepos/js1",
				"build - tsconfig.json",
...
const allTasks: Task[] = (<Task[]>[]).concat(
	...Object.values(tasks).map((v) => Object.values(v))
);
