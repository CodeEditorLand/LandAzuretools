...
export async function confirmOverwriteSettings(
	...
	sourceSettings: { [key: string]: string },
	destinationSettings: { [key: string]: string },
	...
	for (const srcKey of Object.keys(sourceSettings)) {
		...
		const destKey =
			Object.keys(destinationSettings).find(
				(dk) => srcKey.toLowerCase() === dk.toLowerCase()
			) || srcKey;
	...
	if (matchingKeys.length > 0) {
		ext.outputChannel.appendLog(
			l10n.t("Ignored the following settings that were already the same:")
		);
	...
	if (
		Object.keys(destinationSettings).length >
		Object.keys(sourceSettings).length
	) {
		ext.outputChannel.appendLog(
			l10n.t(
				'WARNING: This operation will not delete any settings in "{0}". You must manually delete settings if desired.',
				destinationName
			)
		);
	}
