...
export async function getDeployFsPath(
	...
	target: vscode.Uri | string | AzExtTreeItem | undefined,
	...
	if (!originalDeployFsPath || !effectiveDeployFsPath) {
		...
		const selectFile: string = vscode.l10n.t(
		...
		const selectedItem = fileExtensions
			? await workspaceUtil.selectWorkspaceFile(
					context,
					selectFile,
					fileExtensions,
				)
...
async function addRuntimeFileTelemetry(
	context: IActionContext,
	...
	const tasks: Promise<void>[] = [
		...["package.json", "requirements.txt", "pom.xml"].map((f) =>
			checkRuntimeFile(runtimeFiles, effectiveDeployFsPath, f),
		),
		...["dll", "jar", "war", "csproj", "fsproj"].map((e) =>
...
async function checkRuntimeFile(
	runtimeFiles: string[],
	effectiveDeployFsPath: string,
	fileName: string,
): Promise<void> {
...
async function checkRuntimeExtension(
	runtimeFiles: string[],
	effectiveDeployFsPath: string,
	extension: string,
): Promise<void> {
	const files = await vscode.workspace.findFiles(
		new vscode.RelativePattern(effectiveDeployFsPath, `*.${extension}`),
		undefined,
		1 /* maxResults */,
	);
 ...
 */
async function appendDeploySubpathSetting(
	context: IActionContext,
	...
	if (workspaceFolder) {
		...
		if (deploySubPath) {
			...
			if (isPathEqual(workspaceFolder.uri.fsPath, targetPath)) {
				...
				const fsPathWithSetting: string = path.join(
				...
				if (!isPathEqual(fsPathWithSetting, targetPath)) {
					...
					if (getWorkspaceSetting(settingKey, ext.prefix)) {
						...
						void context.ui
							...
							.then(async (result: vscode.MessageItem) => {
...
function promptToOpenWorkspace(
	context: IActionContext,
	originalDeployFsPath: string,
): never {
	...
	// don't wait
	void context.ui
		.showWarningMessage(message, openInNewWindow)
		.then(async (result) => {
			await callWithTelemetryAndErrorHandling(
				"deployWarning.openInNewWindow",
				async (postDeployContext: IActionContext) => {
					...
					if (result === openInNewWindow) {
						await vscode.commands.executeCommand(
							"vscode.openFolder",
							vscode.Uri.file(originalDeployFsPath),
							true /* forceNewWindow */,
						);
					}
				},
