...
export async function localGitDeploy(
	site: ParsedSite,
	...
	await callWithMaskHandling(async (): Promise<void> => {
		const remote: string = `https://${encodeURIComponent(
			publishingUserName
		)}:${encodeURIComponent(publishingPassword)}@${site.gitUrl}`;
			...
			if (err.message.indexOf("spawn git ENOENT") >= 0) {
				...
				const input: string | undefined =
					await vscode.window.showErrorMessage(
						vscode.l10n.t(
							"Git must be installed to use Local Git Deploy."
						),
						installString
					);
		...
		async function tryPushAndWaitForDeploymentToComplete(
			...
			try {
				...
				await new Promise<void>(
					(
						...
						const pushOptions: Options = forcePush
							? { "-f": null }
							: {};
						...
						waitForDeploymentToComplete(context, site, {
							expectedId: commitId,
							token,
						})
							.then(resolve)
