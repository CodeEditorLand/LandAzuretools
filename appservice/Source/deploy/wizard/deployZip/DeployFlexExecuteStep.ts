...
export class DeployFlexExecuteStep extends DeployZipBaseExecuteStep {
	public async deployZip(
		...
		const site = await this.getFlexSite(
			context,
			context.site.subscription.subscriptionId,
	...
	private async tryCreateStorageContainer(
		context: InnerDeployContext,
		site: Site & { properties?: { functionAppConfig: FunctionAppConfig } },
	): Promise<void> {
	...
	private async getFlexSite(
		context: InnerDeployContext,
		subscriptionId: string,
		rgName: string,
		siteName: string,
	): Promise<
		Site & { properties?: { functionAppConfig: FunctionAppConfig } }
	> {
