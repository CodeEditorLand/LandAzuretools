...
export async function runWithZipStream(
	...
	function onFileSize(size: number): void {
		...
		ext.outputChannel.appendLog(
			vscode.l10n.t("Zip package size: {0}", prettybytes(size)),
	...
	if (getFileExtension(fsPath) === "zip") {
		...
		ext.outputChannel.appendLog(vscode.l10n.t("Creating zip package..."), {
			resourceName: site.fullName,
		});
		...
		if ((await fse.lstat(fsPath)).isDirectory()) {
			...
			zipFile.addFile(
				fsPath,
				getPathFromMap(path.basename(fsPath), pathFileMap)
			);
		}
...
async function getFilesFromGlob(
	...
	const zipDeployConfig: vscode.WorkspaceConfiguration =
	...
	let ignorePatternList: string | string[] =
		zipDeployConfig.get<string | string[]>(zipIgnorePatternStr) || "";
	const filesToIgnore: string[] = await globby(
		ignorePatternList,
		globOptions
	);
 ...
 */
async function getFilesFromGitignore(
	folderPath: string,
	...
	if (await fse.pathExists(gitignorePath)) {
		const funcIgnoreContents: string = (
			await fse.readFile(gitignorePath)
		).toString();
