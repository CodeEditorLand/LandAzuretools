	...
	if (getFileExtension(fsPath) === "zip") {
		...
		if ((await fse.lstat(fsPath)).isDirectory()) {
			...
			if (site.isFunctionApp) {
				...
				filesToZip = await getFilesFromGlob(
					fsPath,
					site.fullName,
					options.progress,
				);
 ...
 */
export async function getFilesFromGlob(
	folderPath: string,
	resourceName: string,
	progress?: vscode.Progress<{ message?: string; increment?: number }>,
): Promise<string[]> {
	...
	if (ignorePatternList) {
		...
		for (const pattern of ignorePatternList) {
			const filesIgnored = (
				await vscode.workspace.findFiles(globPattern, pattern)
			).map((uri) => uri.fsPath);
 ...
 */
export async function getFilesFromGitignore(
	folderPath: string,
