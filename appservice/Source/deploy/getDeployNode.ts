...
function isAzExtTreeItem(ti: unknown): ti is AzExtTreeItem {
	return (
		!!ti &&
		(ti as AzExtTreeItem).fullId !== undefined &&
		(ti as AzExtTreeItem).fullId !== null
	);
}
 ...
 */
export async function getDeployNode<T extends AzExtTreeItem>(
	...
	if (isAzExtTreeItem(arg1)) {
		...
		const defaultAppId: string | undefined = getWorkspaceSetting(
		...
		if (defaultAppId && defaultAppId.toLowerCase() !== "none") {
			...
			if (node) {
				...
				ext.outputChannel.appendLog(
					vscode.l10n.t(
						'WARNING: Failed to find app matching setting "{0}.{1}" with id "{2}"',
						ext.prefix,
						context.defaultAppSetting,
						defaultAppId,
					),
