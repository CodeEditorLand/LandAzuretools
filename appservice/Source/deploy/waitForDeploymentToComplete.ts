...
import { CancellationToken, l10n, Progress, window } from "vscode";
...
export async function waitForDeploymentToComplete(
	context: IActionContext & Partial<InnerDeployContext>,
	site: ParsedSite,
	...
	while (!token?.isCancellationRequested) {
		if (locationUrl) {
			...
			[deployment, permanentId, initialStartTime] =
				await tryGetLatestDeployment(
					context,
					kuduClient,
					permanentId,
					initialStartTime,
		...
		await retryKuduCall(context, "getLogEntry", async () => {
			await ignore404Error(context, async () => {
				logEntries = (
					await kuduClient.getLogEntry(context, deploymentId)
				).reverse();
			});
		});
			...
			await retryKuduCall(context, "getLogEntryDetails", async () => {
				await ignore404Error(context, async () => {
					if (newEntry.id && newEntry.detailsUrl) {
						const details: KuduModels.LogEntry[] =
							await kuduClient.getLogEntryDetails(
								context,
								deploymentId,
								newEntry.id,
							);
		...
		if (deployment.status !== 0) {
			if (deployment.status === 3 /* Failed */ || deployment.isTemp) {
				...
				throw new Error(
					l10n.t(
						"Deployment was cancelled and another deployment is in progress",
					),
				);
			...
			context.syncTriggersPostDeploy =
				site.isFunctionApp &&
	...
	> {
		...
		if (permanentId) {
			...
			deployment = await retryKuduCall(context, "getResult", async () => {
				return await kuduClient.getDeployResult(
					context,
					nonNullValue(permanentId),
				);
			});
		} else if (expectedId) {
			// if we have a "expectedId" we know which deployment we are looking for, so wait until latest id reflects that
			try {
				const latestDeployment: KuduModels.DeployResult =
					await retryKuduCall(context, "getResult", async () => {
						return await kuduClient.getDeployResult(
							context,
							"latest",
						);
			...
			deployment = deployments
				...
				.filter(
					(deployResult) =>
						deployResult.startTime &&
						deployResult.startTime >= initialStartTime!,
				)
				.sort(
					(a, b) =>
						nonNullProp(b, "startTime").valueOf() -
			...
			try {
				deployment = await retryKuduCall(
					context,
					"getResult",
					async () => {
						return await kuduClient.getDeployResult(
