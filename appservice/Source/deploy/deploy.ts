...
export async function deploy(
	...
	try {
		...
		context.telemetry.properties.pythonVersion = String(
		...
		aspPromise.then(
			(plan: AppServicePlan | undefined) => {
				if (plan) {
					context.telemetry.properties.planStatus = String(
						plan.status
					);
	...
	const title: string = l10n.t(
		'Deploying to "{0}"... Check [output window](command:{1}) for status.',
		site.fullName,
	...
	await window.withProgress(
		...
		async () => {
			if (context.stopAppBeforeDeploy) {
				ext.outputChannel.appendLog(l10n.t("Stopping app..."), {
					resourceName: site.fullName,
				});
				await client.stop();
			}

			ext.outputChannel.appendLog(l10n.t("Starting deployment..."), {
				resourceName: site.fullName,
			});
			try {
				if (
					!context.deployMethod &&
					config.scmType === ScmType.GitHub
				) {
					throw new Error(
						l10n.t(
					...
					!context.deployMethod &&
					config.scmType === ScmType.LocalGit
				) {
					...
					if (!(await fse.pathExists(fsPath))) {
						throw new Error(
							l10n.t(
								"Failed to deploy path that does not exist: {0}",
								fsPath
							)
						);
					}

					const javaRuntime = site.isLinux
						? config.linuxFxVersion
						: config.javaContainer;
					if (
						javaRuntime &&
						/^(tomcat|wildfly|jboss)/i.test(javaRuntime)
					) {
						await deployWar(context, site, fsPath);
					} else if (
						javaRuntime &&
						/^java/i.test(javaRuntime) &&
						!site.isFunctionApp
					) {
				...
				if (context.stopAppBeforeDeploy) {
					ext.outputChannel.appendLog(l10n.t("Starting app..."), {
						resourceName: site.fullName,
					});
					await client.start();
				}
