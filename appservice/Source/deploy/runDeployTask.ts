...
export async function runPreDeployTask(
	context: IDeployContext,
	deployFsPath: string,
	scmType: string | undefined
): Promise<void> {
	const preDeployResult: IPreDeployTaskResult = await tryRunPreDeployTask(
		context,
		deployFsPath,
		scmType
	);
	if (preDeployResult.failedToFindTask) {
		...
		preDeployResult.exitCode !== undefined &&
		preDeployResult.exitCode !== 0
	) {
		await handleFailedPreDeployTask(context, preDeployResult);
	}
}

export async function tryRunPreDeployTask(
	context: IDeployContext,
	deployFsPath: string,
	scmType: string | undefined
	...
	let preDeployTaskResult: IPreDeployTaskResult = {
		taskName,
		exitCode: undefined,
		failedToFindTask: false,
	};

	if (taskName && shouldExecuteTask(context, scmType, settingKey, taskName)) {
		const task: vscode.Task | undefined = await taskUtils.findTask(
			deployFsPath,
			taskName
		);
 ...
 */
export async function startPostDeployTask(
	context: IDeployContext,
	deployFsPath: string,
	scmType: string | undefined,
	resourceName: string
	...
	if (taskName && shouldExecuteTask(context, scmType, settingKey, taskName)) {
		...
		if (task) {
			...
			ext.outputChannel.appendLog(
				vscode.l10n.t('Started {0} "{1}".', settingKey, taskName),
				{ resourceName }
			);
		} else {
			ext.outputChannel.appendLog(
				vscode.l10n.t(
					'WARNING: Failed to find {0} "{1}".',
					settingKey,
					taskName
				),
				{ resourceName }
...
function shouldExecuteTask(
	...
	if (!shouldExecute) {
		ext.outputChannel.appendLog(
			vscode.l10n.t(
				'WARNING: Ignoring {0} "{1}" for non-zip deploy.',
				settingKey,
				taskName
			)
		);
...
async function waitForPreDeployTask(
	preDeployTask: vscode.Task,
	deployFsPath: string
): Promise<IPreDeployTaskResult> {
	return await new Promise(
		(
			resolve: (preDeployTaskResult: IPreDeployTaskResult) => void
		): void => {
			const errorListener: vscode.Disposable =
...
export async function handleFailedPreDeployTask(
	context: IActionContext,
	preDeployResult: IPreDeployTaskResult
): Promise<void> {
