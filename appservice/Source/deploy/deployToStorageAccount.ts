 ...
 */
export async function deployToStorageAccount(
...
async function createBlobServiceClient(
	...
	if (connectionString) {
		...
		throw new Error(
			vscode.l10n.t(
				'"{0}" app setting is required for Run From Package deployment.',
...
async function createBlobFromZip(
	context: IActionContext,
	fsPath: string,
	site: ParsedSite,
	...
	if (blobService.credential instanceof StorageSharedKeyCredential) {
		...
		throw new Error(
			'Internal Error: Expected credential to be of type "StorageSharedKeyCredential".',
		);
	}
}
