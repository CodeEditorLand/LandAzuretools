...
export async function startStreamingLogs(
	...
	site: ParsedSite,
	verifyLoggingEnabled: () => Promise<void>,
				...
				void callWithTelemetryAndErrorHandling(
					...
					async (streamContext: IActionContext) => {
						...
						const genericClient: ServiceClient =
							await createGenericClient(streamContext, undefined);
						...
						const logsResponse: AzExtPipelineResponse =
							await genericClient.sendRequest(
								createPipelineRequest({
									...
									streamResponseStatusCodes: new Set([
										200, 206,
									]),
								}),
							);
...
export async function stopStreamingLogs(
	...
	if (logStream && logStream.isConnected) {
		...
		await vscode.window.showWarningMessage(
			vscode.l10n.t("The log-streaming service is already disconnected."),
		);
	}
}
