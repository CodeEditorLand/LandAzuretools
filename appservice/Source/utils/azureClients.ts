...
export async function createAppInsightsClient(
	context: AzExtClientContext,
): Promise<ApplicationInsightsManagementClient> {
	return createAzureClient(
		context,
		(await import("@azure/arm-appinsights"))
...
export async function createResourceGraphClient(
	context: AzExtClientContext,
): Promise<ResourceGraphClient> {
	return createAzureSubscriptionClient(
		context,
		(await import("@azure/arm-resourcegraph")).ResourceGraphClient,
	);
}
