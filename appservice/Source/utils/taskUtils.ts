...
import { tasks as codeTasks, Task, TaskScope, WorkspaceFolder } from "vscode";
...
export namespace taskUtils {
	...
	export function isTaskInScopeOfPath(task: Task, fsPath: string): boolean {
		if (
			...
			return (
				!!taskPath &&
				(isPathEqual(taskPath, fsPath) || isSubpath(taskPath, fsPath))
			);
		}
	}
	...
	export async function findTask(
		...
		return tasks.find((t) => {
			return (
				isTaskInScopeOfPath(t, deployFsPath) &&
				(taskName === t.name.toLowerCase() ||
					taskName ===
						`${t.definition.type}: ` + t.name.toLowerCase() ||
