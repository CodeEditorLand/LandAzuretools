...
import { createPipelineRequest, RestError } from "@azure/core-rest-pipeline";
import {
 ...
 */
export function createSiteFilesUrl(
	site: ParsedSite,
	path: string,
	href?: string,
): string {
...
export async function listFiles(
	...
	site: ParsedSite,
	url: string,
): Promise<ISiteFileMetadata[]> {
	const response: AzExtPipelineResponse = await getFsResponse(
		context,
		site,
		url,
	);
					...
					try {
						...
						if (
							!(
								...
								serviceUnavailable.test(parsedError.message)
							)
						) {
