...
export async function createSlot(
	...
	const slotName: string = (
		await context.ui.showInputBox({
			...
			validateInput: async (value: string): Promise<string | undefined> =>
				validateSlotName(value, gClient, site),
		})
	).trim();
	...
	const configurationSource = await chooseConfigurationSource(
		context,
		site,
		existingSlots
	);
...
async function validateSlotName(
	value: string,
	...
	if (value === "production") {
		...
		return l10n.t(
			"The slot name must be at least {0} characters.",
			slotNamingRules.minLength
		);
	} else if (
		value.length + site.siteName.length >
		slotNamingRules.maxLength
	) {
		return l10n.t(
			"The combined site name and slot name must be fewer than {0} characters.",
			slotNamingRules.maxLength
		);
		...
		const nameAvailability: CheckNameAvailabilityResponse =
			await checkNameAvailability(
				client,
				site.subscription.subscriptionId,
				`${site.siteName}-${value}`,
				"Slot"
			);
...
async function chooseConfigurationSource(
	context: IActionContext,
	site: ParsedSite,
	existingSlots: ParsedSite[]
): Promise<ParsedSite | undefined> {
	if (site.isFunctionApp) {
		// Function apps always clone from production slot without prompting
		return site;
	} else {
		const configurationSources: IAzureQuickPickItem<
