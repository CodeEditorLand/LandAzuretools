...
export class DeploymentTreeItem extends AzExtTreeItem {
	...
	public get contextValue(): string {
		return createContextValue([
			`deployment/${this._scmType}`.toLocaleLowerCase(),
	...
	public async redeployDeployment(context: IActionContext): Promise<void> {
		...
		await window.withProgress(
			...
			async (): Promise<void> => {
				...
				const refreshingInteveral: NodeJS.Timer = setInterval(
					async () => {
						await this.refresh(context);
					},
					1000
				); /* the status of the label changes during deployment so poll for that*/
				try {
					await waitForDeploymentToComplete(
						context,
						this.parent.site,
						{ expectedId: this.id }
					);
	...
	public async viewCommitInGitHub(context: IActionContext): Promise<void> {
		...
		if (sourceControl.repoUrl) {
			...
			throw new Error(
				l10n.t(
					'There is no GitHub repo url associated with deployment "{0}".',
					this._deployResult.id!
				)
			);
		}
	}
	...
	private formatLogEntry(logEntry: KuduModels.LogEntry): string {
		if (logEntry.logTime && logEntry.message) {
			return `${logEntry.logTime.toISOString()} - ${logEntry.message}${
				os.EOL
			}`;
		} else {
			return "";
