...
export async function setRemoteDebug(
	context: IActionContext,
	isRemoteDebuggingToBeEnabled: boolean,
	confirmMessage: string,
	noopMessage: string | undefined,
	site: ParsedSite,
	...
	if (state && state.toLowerCase() === "stopped") {
		throw new Error(
			vscode.l10n.t(
				'The app must be running, but is currently in state "Stopped". Start the app to continue.'
			)
		);
	}

	if (isRemoteDebuggingToBeEnabled !== siteConfig.remoteDebuggingEnabled) {
		const confirmButton: vscode.MessageItem = isRemoteDebuggingToBeEnabled
			? { title: "Enable" }
			: { title: "Disable" };
		...
		reportMessage(
			vscode.l10n.t("Updating site configuration done."),
			progress,
			token
		);
	} else {
