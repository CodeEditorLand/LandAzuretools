...
export class TunnelProxy {
	...
	private async checkTunnelStatus(context: IActionContext): Promise<void> {
		...
		if (tunnelStatus.state === AppState.STARTED) {
			if (
				(tunnelStatus.port === 2222 && !this._isSsh) ||
				(tunnelStatus.port !== 2222 && this._isSsh)
			) {
				...
				throw new Error(
					l10n.t("App is started, but port is unreachable"),
				);
			}
			...
			throw new Error(
				l10n.t("Unexpected app state: {0}", tunnelStatus.state),
			);
		}
	}

	private async checkTunnelStatusWithRetry(
		context: IActionContext,
		token: CancellationToken,
	): Promise<void> {
		...
		while (Date.now() < start + timeoutMs) {
			...
			try {
				...
				if (!(error instanceof RetryableTunnelStatusError)) {
					throw new Error(
						l10n.t(
							"Unable to establish connection to application: {0}",
	...
	private async setupTunnelServer(
		bearerToken: string,
		token: CancellationToken,
	): Promise<void> {
		return new Promise<void>(
			(resolve: () => void, reject: (err: Error) => void): void => {
				...
				this._server.on("connection", (socket: Socket) => {
					...
					ext.outputChannel.appendLog(
						`[Proxy Server] client connected ${socket.remoteAddress}:${socket.remotePort}, connection count: ${this._openSockets.length}`,
					);
				});
