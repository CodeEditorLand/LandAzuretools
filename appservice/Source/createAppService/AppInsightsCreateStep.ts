...
export class AppInsightsCreateStep extends AzureWizardExecuteStep<IAppServiceWizardContext> {
	...
	public async execute(
		...
		if (appInsightsLocation) {
			...
			try {
				context.appInsightsComponent = await client.components.get(
					rgName,
					aiName
				);
				...
				if (pError.errorType === "ResourceNotFound") {
					const creatingNewAppInsights: string = l10n.t(
					...
					context.appInsightsComponent =
						await client.components.createOrUpdate(rgName, aiName, {
							kind: "web",
							applicationType: "web",
	...
	public async selectExistingPrompt(
		context: IAppServiceWizardContext
	): Promise<void> {
