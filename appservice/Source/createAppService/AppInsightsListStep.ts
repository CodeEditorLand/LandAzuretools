...
export class AppInsightsListStep extends AzureWizardPromptStep<IAppServiceWizardContext> {
	...
	public static async getAppInsightsComponents(
	...
	public async getSubWizard(
		...
		if (context.appInsightsComponent) {
			context.valuesToMask.push(
				nonNullProp(context.appInsightsComponent, "name"),
			);
	...
	private async getQuickPicks(
		...
		IAzureQuickPickItem<ApplicationInsightsComponent | undefined>[]
	> {
		const picks: IAzureQuickPickItem<
			ApplicationInsightsComponent | undefined
		>[] = !this._suppressCreate
			? [
					{
						label: vscode.l10n.t(
							"$(plus) Create new Application Insights resource",
						),
						data: undefined,
					},
				]
		...
		let components: ApplicationInsightsComponent[] =
		...
		return picks.concat(
			components.map((ai: ApplicationInsightsComponent) => {
				return {
					id: ai.id,
