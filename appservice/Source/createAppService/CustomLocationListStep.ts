...
} from "./IAppServiceWizardContext";

export class CustomLocationListStep<
	T extends IAppServiceWizardContext,
> extends LocationListStep<T> {
	public async prompt(context: T): Promise<void> {
		...
		if ("kubeEnvironment" in result) {
			...
			await LocationListStep.setLocation(
				context,
				result.kubeEnvironment.location,
			);
	...
	protected async getCustomQuickPicks(
		context: T,
		...
		if (context.newSiteOS !== "windows") {
			try {
				...
				context.telemetry.properties.customLocationError =
