...
export class AppServicePlanListStep extends AzureWizardPromptStep<IAppServiceWizardContext> {
	...
	public async prompt(context: IAppServiceWizardContext): Promise<void> {
		...
		const options: IAzureQuickPickOptions = {
			placeHolder:
				context.newSiteKind?.includes(AppKind.workflowapp) &&
				context.planSkuFamilyFilter?.test("IV2")
					? vscode.l10n.t(
							"Select an App Service Environment (v3) Plan",
						)
					: vscode.l10n.t(
							"Select a {0} App Service plan.",
							getWebsiteOSDisplayName(
								nonNullProp(context, "newSiteOS"),
							),
						),
	...
	public async getSubWizard(
		context: IAppServiceWizardContext,
	): Promise<IWizardOptions<IAppServiceWizardContext> | undefined> {
		if (!context.plan) {
			const promptSteps: AzureWizardPromptStep<IAppServiceWizardContext>[] =
				[
					new AppServicePlanNameStep(),
					new AppServicePlanSkuStep(),
	...
	private async getQuickPicks(
		context: IAppServiceWizardContext,
	): Promise<IAzureQuickPickItem<AppServicePlan | undefined>[]> {
		const picks: IAzureQuickPickItem<AppServicePlan | undefined>[] = !this
			._suppressCreate
			? [
					{
						label: vscode.l10n.t(
							"$(plus) Create new App Service plan",
						),
						description: "",
						data: undefined,
					},
		...
		if (famFilter) {
			plans = plans.filter(
				(plan) =>
					!plan.sku ||
					!plan.sku.family ||
					famFilter.test(plan.sku.family),
			);
		}
		...
		for (const plan of plans) {
			...
			if (
				plan.sku &&
				(plan.sku.family === "EP" || plan.sku.family === "WS")
			) {
		...
		if (hasFilteredLocations && location) {
			picks.push({
				label: vscode.l10n.t(
					'$(warning) Only plans in the region "{0}" are shown.',
