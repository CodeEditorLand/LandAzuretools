...
export class AppServicePlanRedundancyStep extends AzureWizardPromptStep<IAppServiceWizardContext> {
	...
	public static isZoneRedundancySupportedServicePlan(
		newPlanSkuOrFamily: SkuDescription | string,
	): boolean {
	...
	public static isZoneRedundancySupported(
		location: string,
		newPlanSkuOrFamily: SkuDescription | string,
	): boolean {
		return (
			this.isZoneRedundancySupportedLocation(location) &&
			this.isZoneRedundancySupportedServicePlan(newPlanSkuOrFamily)
		);
	}
	...
	public shouldPrompt(context: AppServiceWizardContext): boolean {
		...
		if (
			...
			return AppServicePlanRedundancyStep.isZoneRedundancySupported(
				name,
				newPlanSku,
			);
		}
