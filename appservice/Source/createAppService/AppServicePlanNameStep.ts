...
export class AppServicePlanNameStep extends AzureWizardPromptStep<IAppServiceWizardContext> {
		...
		context.newPlanName = (
			await context.ui.showInputBox({
				...
				prompt: vscode.l10n.t(
					"Enter the name of the new App Service plan."
				),
				validateInput: async (
					value: string
	...
	private async validatePlanName(
		...
		if (
			name.length < appServicePlanNamingRules.minLength ||
			name.length > appServicePlanNamingRules.maxLength
		) {
			return vscode.l10n.t(
				"The name must be between {0} and {1} characters.",
				appServicePlanNamingRules.minLength,
				appServicePlanNamingRules.maxLength
			);
			...
			!(await AppServicePlanListStep.isNameAvailable(
				context,
				name,
				nonNullProp(context.resourceGroup, "name")
			))
		) {
			// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
			return vscode.l10n.t(
				'App Service plan "{0}" already exists in resource group "{1}".',
				name,
