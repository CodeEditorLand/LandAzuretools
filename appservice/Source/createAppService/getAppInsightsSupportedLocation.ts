...
export async function getAppInsightsSupportedLocation(
	...
	if (locations.some((loc) => areLocationNamesEqual(loc, location.name))) {
		...
		const pairedRegions: string[] | undefined = await getPairedRegions(
			context,
			locationName,
		);
...
async function getPairedRegions(
	...
	try {
		...
		const regionMappingJson: RegionMappingJsonResponse = <
			RegionMappingJsonResponse
		>response.parsedBody;
