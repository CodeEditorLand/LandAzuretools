...
import {
	AppInsightsListStep,
	appInsightsNamingRules,
} from "./AppInsightsListStep";
...
export class AppInsightsNameStep extends AzureWizardPromptStep<IAppServiceWizardContext> {
	...
	private async validateApplicationInsightName(
		context: IAppServiceWizardContext,
		name: string
	): Promise<string | undefined> {
		name = name.trim();
		...
		if (
			name.length < appInsightsNamingRules.minLength ||
			name.length > appInsightsNamingRules.maxLength
		) {
			return vscode.l10n.t(
				"The name must be between {0} and {1} characters.",
				appInsightsNamingRules.minLength,
				appInsightsNamingRules.maxLength
			);
			...
			return vscode.l10n.t(
				'Application Insights resource "{0}" already exists in subscription "{1}".',
				name,
