...
export class AppServicePlanSkuStep extends AzureWizardPromptStep<IAppServiceWizardContext> {
	public async prompt(context: IAppServiceWizardContext): Promise<void> {
		...
		if (context.newSiteKind === AppKind.functionapp) {
			...
			context["newSiteJavaStack"]?.majorVersion?.value === "jbosseap"
		) {
		...
		const pricingTiers: IAzureQuickPickItem<SkuDescription | undefined>[] =
			skus.map((s) => {
				return {
		...
		pricingTiers.push({
			label: vscode.l10n.t(
				"$(link-external) Show pricing information...",
			),
			data: undefined,
