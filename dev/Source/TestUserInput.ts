...
export class TestUserInput implements types.TestUserInput {
	...
	public async runWithInputs<T>(
		inputs: (string | RegExp | types.TestInput)[],
		callback: () => Promise<T>,
	): Promise<T> {
	...
	public validateAllInputsUsed(): void {
		assert.strictEqual(
			this._inputs.length,
			0,
			`Not all inputs were used: ${this._inputs.toString()}`,
		);
	}

	public async showQuickPick<T extends vscodeTypes.QuickPickItem>(
		items: T[] | Thenable<T[]>,
		options: vscodeTypes.QuickPickOptions,
	): Promise<T | T[]> {
		const resolvedItems: T[] = await Promise.resolve(items);

		let result: T | T[];

		const input: string | RegExp | TestInput | undefined =
			this._inputs.shift();

		if (input === undefined) {
			throw new Error(
				`No more inputs left for call to showQuickPick. Placeholder: '${options.placeHolder}'`,
			);
			...
			if (resolvedItems.length === 0) {
				throw new Error(
					`No quick pick items found. Placeholder: '${options.placeHolder}'`,
				);
			} else if (input === TestInput.UseDefaultValue) {
				result = resolvedItems[0];
			} else {
				function qpiMatchesInput(
					qpi: vscodeTypes.QuickPickItem,
				): boolean {
					...
					const valuesToTest = [
						qpi.label,
						description,
						`${qpi.label} ${description}`,
					];

					return valuesToTest.some((v) =>
						input instanceof RegExp ? input.test(v) : input === v,
					);
				}
				...
				if (options.canPickMany) {
					...
					const resolvedItem: T | undefined =
						resolvedItems.find(qpiMatchesInput);
	...
	public async showInputBox(
		options: vscodeTypes.InputBoxOptions,
	): Promise<string> {
		let result: string;
		...
		if (input === undefined) {
			...
			if (!options.value) {
				throw new Error(
					"Can't use default value because none was specified",
				);
			} else {
			...
			if (options.validateInput) {
				const msg:
					| string
					| vscodeTypes.InputBoxValidationMessage
					| null
					| undefined = await Promise.resolve(
	...
	public showWarningMessage<T extends vscodeTypes.MessageItem>(
		...
		...items: T[]
	): Promise<vscodeTypes.MessageItem>;
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	public async showWarningMessage<T extends vscodeTypes.MessageItem>(
		message: string,
		...args: any[]
	): Promise<T> {
		let result: T;
		...
		if (input === undefined) {
			throw new Error(
				`No more inputs left for call to showWarningMessage. Message: ${message}`,
			);
		} else if (typeof input === "string") {
			// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
			const matchingItem: T | undefined = args.find(
				(item: T) => item.title === input,
			);
			...
			if (matchingItem) {
				...
				throw new Error(
					`Did not find message item matching '${input}'. Message: '${message}'`,
				);
			}
		} else {
			throw new Error(
				`Unexpected input '${input}' for showWarningMessage.`,
			);
		}
	...
	public async showOpenDialog(
		options: vscodeTypes.OpenDialogOptions,
		...
		if (input === undefined) {
			throw new Error(
				`No more inputs left for call to showOpenDialog. Message: ${options.openLabel}`,
			);
		} else if (typeof input === "string") {
	...
	public async showWorkspaceFolderPick(
		options: vscodeTypes.WorkspaceFolderPickOptions,
		...
		if (input === undefined) {
			...
			const workspaceFolders:
				| readonly vscodeTypes.WorkspaceFolder[]
				| undefined = this._vscode.workspace.workspaceFolders;

			const workspaceFolder: vscodeTypes.WorkspaceFolder | undefined =
			...
			throw new Error(
				`Unexpected input '${input}' for showWorkspaceFolderPick.`,
			);
		}
...
export async function runWithInputs<T>(
	callbackId: string,
	inputs: (string | RegExp | types.TestInput)[],
	registerOnActionStartHandler: types.registerOnActionStartHandlerType,
	callback: () => Promise<T>,
): Promise<T> {
