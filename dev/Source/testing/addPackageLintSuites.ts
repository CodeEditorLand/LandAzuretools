...
export function addPackageLintSuites(
	...
	const commandsRegisteredButNotInPackage: string[] =
	...
	const extensionPrefixWithPeriod: string = commands[0].command.substr(
		0,
		commands[0].command.indexOf(".") + 1
	);

	function verifyStartsWithExtensionPrefix(name: string): void {
		assert(
			name.startsWith(extensionPrefixWithPeriod),
			`Expected ${name} to start with ${extensionPrefixWithPeriod}`
		);
	}
			...
			for (const view of viewContainer) {
				...
				test(view.id, () => {
					assert(
						...
						`Couldn't find activation event ${activationEvent}`
					);
				});
			}
		}
		...
		for (const cmd of commands) {
			...
			test(cmdId, async () => {
				...
				assert(
					...
					`Couldn't find activation event for command ${cmdId}`
				);
			});
		}
			...
			if (cmdId.startsWith(extensionPrefixWithPeriod)) {
				...
				if (
					...
					assert(
						...
						`${cmdId} is in commandsRegisteredButNotInPackage but has an activation event`
					);
				} else {
					assert(
						isInPackage,
					...
					assert(
						...
						`Couldn't find activation event for registered command ${cmdId}`
					);
				}
			}
		}
