...
export function excludeNodeModulesAndDependencies(
	...
	moduleNames: string[],
	log: (...args: unknown[]) => void = (): void => {
		/* noop */
	},
): void {
	const externalModulesClosure: string[] = getNodeModulesDependencyClosure(
		packageLockJson,
		moduleNames,
	);
				...
				await Promise.all(
					copyEntries.map(async (ce) => {
						...
						if (await fse.pathExists(ce.source)) {
							...
							log(
								`${pluginName}: Ignoring "${entryName}" because it doesn't exist`,
							);
						}
					}),
				);
 ...
 */
export function getNodeModulesDependencyClosure(
	...
	for (const moduleName of moduleNames) {
		if (getDependency(moduleName)) {
			...
			const subdeps: string[] = getDependenciesFromEntry(
			...
			throw new Error(
				`Could not find dependency entry for ${moduleName}`,
			);
		}
	}
...
function getDependenciesFromEntry(
	...
	for (const moduleName of Object.keys(dependencies)) {
		// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
		const dependenciesSubdeps: string[] = getDependenciesFromEntry(
			dependencies[moduleName]!,
			packageLock,
		);
...
export function getExternalsEntries(moduleNames: string[]): {
	[moduleName: string]: string;
} {
...
export function getNodeModuleCopyEntries(
	projectRoot: string,
	moduleNames: string[],
): CopyEntry[] {
