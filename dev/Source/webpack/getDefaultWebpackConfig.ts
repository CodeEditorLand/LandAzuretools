...
} from "./excludeNodeModulesAndDependencies";
...
export function getDefaultWebpackConfig(
	...
	const packageLockJson: PackageLock = fse.readJsonSync(
		path.resolve(options.projectRoot, "package-lock.json")
	);
	const existingDefaultExtNodeModules: string[] =
		defaultExternalNodeModules.filter(
			(moduleName: string) =>
				packageLockJson.dependencies &&
				!!packageLockJson.dependencies[moduleName]
		);
	...
	const nodeCryptoPath = path.resolve(
		__dirname,
		"..",
		"..",
		"..",
	...
	const webCryptoPath = path.resolve(
		__dirname,
		"..",
		"..",
		"..",
	...
	const config: webpack.Configuration = {
		...
		output: {
			...
			path:
				options.target === "webworker"
					? path.resolve(options.projectRoot, "dist", "web")
		...
		resolve: {
			mainFields:
				options.target === "webworker"
					? ["browser", "module", "main"]
					: ["module", "main"], // look for `browser` entry point in imported node modules
			...
			extensions: [".ts", ".js"],
			alias: options.target === "webworker" ? alias : undefined,
			fallback:
				options.target === "webworker"
					? {
