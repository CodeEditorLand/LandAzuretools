...
suite("request", () => {
	...
	async function sendTestRequest(
		...
		return await sendRequestWithTimeout(
			await createTestActionContext(),
			{ method: "GET", url, allowInsecureConnection: true },
			2000,
			undefined
		);
	}
	...
	test("400", async () => {
		await assertThrowsAsync(
			async () => await sendTestRequest({ statusCode: 400 }),
			/400/
		);
	});
	...
	test("400 with error message", async () => {
		await assertThrowsAsync(
			async () =>
				await sendTestRequest({ statusCode: 400, body: "oops" }),
			/oops/
		);
	});
	...
	test("400 with json error message", async () => {
		await assertThrowsAsync(
			...
			(err: Error) =>
				err.message.includes("oops") && !err.message.includes("message")
		);
	});

	test("ECONNRESET", async () => {
		await assertThrowsAsync(
			async () => await sendTestRequest((res) => res.destroy()),
			/socket hang up/i
		);
	});
});
