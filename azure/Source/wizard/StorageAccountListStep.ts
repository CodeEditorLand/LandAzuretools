...
export class StorageAccountListStep<
		T extends types.IStorageAccountWizardContext,
	>
	...
	implements types.StorageAccountListStep<T>
{
	...
	public constructor(
		newAccountDefaults: types.INewStorageAccountDefaults,
	...
	public async prompt(wizardContext: T): Promise<void> {
		...
		> = this.getQuickPicks(
			wizardContext,
			uiUtils.listAllIterator(client.storageAccounts.list()),
		);
		...
		if (wizardContext.storageAccount) {
			...
			await LocationListStep.setLocation(
				wizardContext,
				wizardContext.storageAccount.location!,
			);
		}
	}
	...
	private async getQuickPicks(
		wizardContext: T,
		storageAccountsTask: Promise<StorageAccount[]>,
	): Promise<IAzureQuickPickItem<StorageAccount | undefined>[]> {
		...
		const kindRegExp: RegExp = new RegExp(
		...
		for (const sa of storageAccounts) {
			if (
				!sa.kind ||
				sa.kind.match(kindRegExp) ||
				!sa.sku ||
				sa.sku.name.match(performanceRegExp) ||
		...
		if (hasFilteredAccountsBySku && this._filters.learnMoreLink) {
			picks.push({
				label: vscode.l10n.t(
					"$(info) Some storage accounts were filtered because of their sku. Learn more...",
				),
				onPicked: async () => {
					// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
		...
		if (hasFilteredAccountsByLocation && location) {
			picks.push({
				label: vscode.l10n.t(
					'$(warning) Only storage accounts in the region "{0}" are shown.',
		...
		if (hasFilteredAccountsByNetwork) {
			picks.push({
				label: vscode.l10n.t(
					"$(warning) Some storage accounts were filtered because of their network configurations.",
				),
				onPicked: () => {
					/* do nothing */
				},
