...
} from "./ResourceGroupListStep";

export class ResourceGroupNameStep<T extends types.IResourceGroupWizardContext>
	extends AzureWizardPromptStep<T>
	implements types.ResourceGroupNameStep<T>
{
	...
	private async validateResourceGroupName(
		...
		if (
			name.length < resourceGroupNamingRules.minLength ||
			name.length > resourceGroupNamingRules.maxLength
		) {
			return vscode.l10n.t(
				"The name must be between {0} and {1} characters.",
				resourceGroupNamingRules.minLength,
				resourceGroupNamingRules.maxLength,
			);
		} else if (
			name.match(resourceGroupNamingRules.invalidCharsRegExp) !== null
		) {
			return vscode.l10n.t(
				"The name can only contain alphanumeric characters or the symbols ._-()",
			);
		} else if (name.endsWith(".")) {
			return vscode.l10n.t("The name cannot end in a period.");
		} else if (
			!(await ResourceGroupListStep.isNameAvailable(wizardContext, name))
		) {
			return vscode.l10n.t(
				'Resource group "{0}" already exists in subscription "{1}".',
				name,
