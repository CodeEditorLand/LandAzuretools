...
export class ResourceGroupListStep<T extends types.IResourceGroupWizardContext>
	extends AzureWizardPromptStep<T>
	implements types.ResourceGroupListStep<T>
{
	...
	public static async getResourceGroups<
		T extends types.IResourceGroupWizardContext,
	>(wizardContext: T): Promise<ResourceGroup[]> {
	...
	>(wizardContext: T, name: string): Promise<boolean> {
		...
		return !(await resourceGroupsTask).some(
			(rg: ResourceGroup) =>
				rg.name !== undefined &&
				rg.name.toLowerCase() === name.toLowerCase(),
		);
	}
	...
	public async prompt(wizardContext: T): Promise<void> {
		...
		if (
			...
			await LocationListStep.setLocation(
				wizardContext,
				nonNullProp(wizardContext.resourceGroup, "location"),
			);
		}
	}
	...
	public shouldPrompt(wizardContext: T): boolean {
		return (
			!wizardContext.resourceGroup && !wizardContext.newResourceGroupName
		);
	}

	private async getQuickPicks(
		wizardContext: T,
	): Promise<IAzureQuickPickItem<ResourceGroup | undefined>[]> {
		...
		return picks.concat(
			resourceGroups.map((rg: ResourceGroup) => {
				return {
					id: rg.id,
