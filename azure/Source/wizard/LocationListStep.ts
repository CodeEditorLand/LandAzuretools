...
> extends AzureWizardPromptStep<T> {
		...
		return (await allLocationsTask).filter(
			(l1) =>
				...
				locationSubsets.every((subset) =>
					subset.find(
						(l2) =>
							generalizeLocationName(l1.name) ===
							generalizeLocationName(l2)
					)
				)
		);
	}
	...
	public static locationMatchesName(
		...
		return (
			name === generalizeLocationName(location.name) ||
			name === generalizeLocationName(location.displayName)
		);
	}
	...
	protected async getQuickPicks(
		wizardContext: T
	): Promise<IAzureQuickPickItem<types.AzExtLocation>[]> {
