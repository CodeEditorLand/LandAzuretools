...
export class LocationListStep<
	T extends ILocationWizardContextInternal,
> extends AzureWizardPromptStep<T> {
	protected constructor(private options?: IAzureQuickPickOptions) {
		super();
	}

	public static addStep<T extends ILocationWizardContextInternal>(
		wizardContext: IActionContext & Partial<ILocationWizardContextInternal>,
	...
	private static getInternalVariables<
		T extends ILocationWizardContextInternal,
	>(
		wizardContext: T,
	): [Promise<types.AzExtLocation[]>, Map<string, Promise<string[]>>] {
		...
		if (!wizardContext._providerLocationsMap) {
			wizardContext._providerLocationsMap = new Map<
				string,
				Promise<string[]>
			>();
			// Should be relevant for all our wizards
			this.addProviderForFiltering(
				wizardContext,
				resourcesProvider,
				"resourceGroups",
			);
	...
	public static async setLocation<T extends ILocationWizardContextInternal>(
		wizardContext: T,
		name: string,
	): Promise<void> {
	...
	public static setLocationSubset<T extends ILocationWizardContextInternal>(
		wizardContext: T,
		task: Promise<string[]>,
		provider: string,
	): void {
	...
	public static addProviderForFiltering<
		T extends ILocationWizardContextInternal,
	>(wizardContext: T, provider: string, resourceType: string): void {
		this.setLocationSubset(
			wizardContext,
			getProviderLocations(wizardContext, provider, resourceType),
			provider,
		);
	}

	public static hasLocation<T extends ILocationWizardContextInternal>(
		wizardContext: T,
	): boolean {
	...
	public static async getLocation<T extends ILocationWizardContextInternal>(
		wizardContext: T,
		provider?: string,
		supportsExtendedLocations?: boolean,
	): Promise<types.AzExtLocation> {
		let location: types.AzExtLocation = nonNullProp(
			wizardContext,
			"_location",
		);
		...
		function warnAboutRelatedLocation(loc: types.AzExtLocation): void {
			...
			ext.outputChannel.appendLog(
				vscode.l10n.t(
					'WARNING: Provider "{0}" does not support location "{1}". Using "{2}" instead.',
					provider!,
		...
		if (provider) {
			...
			if (providerLocations) {
				function isSupportedByProvider(
					...
					return !!providerLocations?.find((name) =>
				...
				wizardContext.telemetry.properties.locationProviderNotFound =
					provider;
			}
		}
	...
	public static async getLocations<T extends ILocationWizardContextInternal>(
		wizardContext: T,
	): Promise<types.AzExtLocation[]> {
		const [allLocationsTask, providerLocationsMap] =
			this.getInternalVariables(wizardContext);
		...
		return (await allLocationsTask).filter(
			(l1) =>
				(l1.type === "EdgeZone" &&
					wizardContext.includeExtendedLocations) ||
				locationSubsets.every((subset) =>
					subset.find(
						(l2) =>
							LocationListStep.generalizeLocationName(l1.name) ===
							LocationListStep.generalizeLocationName(l2),
					),
				),
		);
	}
	...
	public static locationMatchesName(
		...
		return (
			...
			name ===
				LocationListStep.generalizeLocationName(location.displayName)
		);
	}
		...
		const options: AgentQuickPickOptions = {
			...
			agentMetadata: {
				...
				parameterDisplayDescription: vscode.l10n.t(
					"The location where resources will be deployed.",
				),
			},
			...this.options,
		};
...
function compareLocation(
	l1: types.AzExtLocation,
	l2: types.AzExtLocation,
): number {
...
class ProviderResourceTypeNotFoundError extends Error {
	constructor(provider: Provider, expectedResourceType: string) {
		super(
			vscode.l10n.t(
				'Provider "{0}" does not have resource type "{1}".',
