...
export type InternalAzExtClientContext =
	| ISubscriptionActionContext
	| [IActionContext, ISubscriptionContext | AzExtTreeItem];

export function parseClientContext(
	clientContext: InternalAzExtClientContext
): ISubscriptionActionContext {
	if (Array.isArray(clientContext)) {
		const subscription =
			clientContext[1] instanceof AzExtTreeItem
				? clientContext[1].subscription
				: clientContext[1];
...
export function createAzureClient<T extends ServiceClient>(
	clientContext: InternalAzExtClientContext,
...
export function createAzureSubscriptionClient<T extends ServiceClient>(
	clientContext: InternalAzExtClientContext,
...
export async function sendRequestWithTimeout(
	...
	options: types.AzExtRequestPrepareOptions,
	timeout: number,
	clientInfo: types.AzExtGenericClientInfo
	...
	const client = await createGenericClient(context, clientInfo, {
		noRetryPolicy: true,
...
export async function createGenericClient(
	context: IActionContext,
	clientInfo: types.AzExtGenericClientInfo | undefined,
...
export function addBasicAuthenticationCredentialsToClient(
	...
	client.pipeline.addPolicy(
		new BasicAuthenticationCredentialsPolicy(userName, password),
		{ phase: "Serialize" }
	);
}
...
class BasicAuthenticationCredentialsPolicy implements PipelinePolicy {
	...
	public async sendRequest(
		...
		const encodedCredentials = `${DEFAULT_AUTHORIZATION_SCHEME} ${Buffer.from(
			credentials
		).toString("base64")}`;
