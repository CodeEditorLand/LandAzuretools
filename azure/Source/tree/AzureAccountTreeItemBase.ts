	...
	implements types.AzureAccountTreeItemBase
{
	...
	public async loadMoreChildrenImpl(
		...
		if (
			...
			return [
				new GenericTreeItem(this, {
					label:
						azureAccount.status === "Initializing"
							? l10n.t("Loading...")
							: l10n.t("Waiting for Azure sign-in..."),
			...
			studentAccountTreeItem.commandArgs = [
		...
		if (azureAccount.filters.length === 0) {
			return [
				new GenericTreeItem(this, {
					label: selectSubscriptionsLabel,
					commandId: selectSubscriptionsCommandId,
					contextValue,
					id: selectSubscriptionsCommandId,
					includeInTreeItemPicker: true,
				}),
			];
		} else {
			this._subscriptionTreeItems = await Promise.all(
				azureAccount.filters.map(
					async (filter: AzureResourceFilter) => {
						const existingTreeItem:
						...
						if (existingTreeItem) {
							...
							addExtensionValueToMask(
								(
									filter.session.credentials2 as unknown as {
							...
							const subscriptionId: string = nonNullProp(
								filter.subscription,
								"subscriptionId",
							);
							...
							return await this.createSubscriptionTreeItem({
								...
								isCustomCloud:
									filter.session.environment.name ===
									"AzureCustomCloud",
							});
						}
					},
	...
	public async getSubscriptionPromptStep(
			...
			class SubscriptionPromptStep extends AzureWizardPromptStep<ISubscriptionActionContext> {
				public async prompt(): Promise<void> {
					const ti: SubscriptionTreeItemBase = <
						SubscriptionTreeItemBase
					>await me.treeDataProvider.showTreeItemPicker(
	...
	public compareChildrenImpl(
		...
		if (
			item1 instanceof GenericTreeItem &&
			item2 instanceof GenericTreeItem
		) {
	...
	private async loadAzureAccount(
		azureAccount: AzureAccountExtensionApi | undefined,
		...
		if (!azureAccount) {
			...
			if (extension) {
				try {
					...
					if (
						semver.lt(
							extension.packageJSON.version as string,
							minAccountExtensionVersion,
						)
					) {
						return "needsUpdate";
	...
	private async ensureSubscriptionTreeItems(
		context: IActionContext,
