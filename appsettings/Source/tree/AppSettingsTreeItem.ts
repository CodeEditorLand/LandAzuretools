...
import {
	...
	IAppSettingsClient,
} from "../IAppSettingsClient";
import { AppSettingTreeItem } from "./AppSettingTreeItem";

export function validateAppSettingKey(
...
export function validateAppSettingValue(value: string): string | undefined {
	if (!value.trim()) {
		return vscode.l10n.t(
			"App setting values cannot be null, undefined or an empty string.",
		);
	}
...
export class AppSettingsTreeItem extends AzExtParentTreeItem {
	...
	constructor(
		...
		clientProvider: AppSettingsClientProvider,
		...
		await Promise.all(
			Object.keys(properties).map(async (key: string) => {
				const appSettingTreeItem: AppSettingTreeItem =
					await AppSettingTreeItem.createAppSettingTreeItem(
						context,
						this,
						key,
						properties[key],
	...
	public async createChildImpl(
		...
		const newKey: string = await context.ui.showInputBox({
			...
			validateInput: (v: string): string | undefined =>
				validateAppSettingKey(settings, client, v),
		});
		...
		return await AppSettingTreeItem.createAppSettingTreeItem(
			context,
			this,
			newKey,
			newValue,
