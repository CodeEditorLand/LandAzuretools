...
import {
	AppSettingsTreeItem,
	validateAppSettingKey,
} from "./AppSettingsTreeItem";
...
export class AppSettingTreeItem extends AzExtTreeItem {
	...
	public static async createAppSettingTreeItem(
		context: IActionContext,
		parent: AppSettingsTreeItem,
		key: string,
		value: string,
	): Promise<AppSettingTreeItem> {
		const ti: AppSettingTreeItem = new AppSettingTreeItem(
			parent,
			key,
			value,
		);
	...
	public get label(): string {
		return this._hideValue
			? `${this._key}=Hidden value. Click to view.`
	...
	public async edit(context: IActionContext): Promise<void> {
		...
		await this.parent.editSettingItem(
			this._key,
			this._key,
			newValue,
			context,
	...
	public async rename(context: IActionContext): Promise<void> {
		...
		const newKey: string = await context.ui.showInputBox({
			...
			validateInput: (v: string): string | undefined =>
				validateAppSettingKey(settings, client, v, oldKey),
		});
	...
	public async deleteTreeItemImpl(context: IActionContext): Promise<void> {
		await context.ui.showWarningMessage(
			`Are you sure you want to delete setting "${this._key}"?`,
