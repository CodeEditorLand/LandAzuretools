...
export async function deleteLinker(
	context: IActionContext & ExecuteActivityContext,
	id: string,
	subscription: ISubscriptionContext | AzureSubscription,
	serviceConnectorName?: string,
	preSteps: AzureWizardPromptStep<IPickLinkerContext>[] = []
): Promise<void> {
	subscription = isAzureSubscription(subscription)
		? createSubscriptionContext(subscription)
		: subscription; // For v1.5 compatibility
	...
	const confirmMessage: string = vscode.l10n.t(
		"Are you sure you want to delete this service connector? This action will delete the service connection but not the service."
	);
	...
	const wizard: AzureWizard<IPickLinkerContext> = new AzureWizard(
		wizardContext,
		{
