...
export class AuthenticationListStep extends AzureWizardPromptStep<ICreateLinkerContext> {
		...
		const picks: IAzureQuickPickItem<AuthInfoBase>[] = [
			{
				...
				data: { authType: KnownAuthType.SystemAssignedIdentity },
			},
			{
				label: vscode.l10n.t("Connection string"),
				data: { authType: KnownAuthType.Secret },
			},
		];
	...
	public async configureBeforePrompt(
		...
		if (context.keyVaultAccount) {
			context.authType = {
				authType: KnownAuthType.SystemAssignedIdentity,
			}; // connection string is not supported for key vault
		}
	}
}
