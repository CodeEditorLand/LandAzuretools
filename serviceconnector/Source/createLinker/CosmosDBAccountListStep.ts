...
export class CosmosDBAccountListStep extends AzureWizardPromptStep<ICreateLinkerContext> {
	public async prompt(context: ICreateLinkerContext): Promise<void> {
		...
		const filteredAccounts = filterDatabaseAccounts(
			accounts.value,
	...
	private async getPicks(
		accounts: DatabaseAccountJsonResponse[],
	): Promise<IAzureQuickPickItem<DatabaseAccountJsonResponse>[]> {
...
async function getCosmosDBDatabaseAccounts(
	context: ICreateLinkerContext,
): Promise<DatabaseJsonResponse> {
	...
	)).parsedBody as DatabaseJsonResponse;
}

function filterDatabaseAccounts(
	accounts: DatabaseAccountJsonResponse[],
	kind: string | undefined,
): DatabaseAccountJsonResponse[] {
