...
export class ClientListStep extends AzureWizardPromptStep<ICreateLinkerContext> {
	public async prompt(context: ICreateLinkerContext): Promise<void> {
		...
		const picks: IAzureQuickPickItem<KnownClientType>[] = [
			...
			{
				label: vscode.l10n.t("SpringBoot"),
				data: KnownClientType.SpringBoot,
			},
		];

		context.clientType = (
			await context.ui.showQuickPick(this.sortTypes(context, picks), {
				placeHolder,
	...
	private sortTypes(
		...
		return types.sort(
			(s1, s2) => getPriority(s1.data) - getPriority(s2.data)
		);
	}
}
