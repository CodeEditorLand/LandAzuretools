 ...
 */
export function getConfiguredAzureEnv(): azureEnv.Environment & {
	...
	if (environmentSettingValue === CloudEnvironmentSettingValue.ChinaCloud) {
		...
		environmentSettingValue === CloudEnvironmentSettingValue.USGovernment
	) {
		return {
		...
		environmentSettingValue === CloudEnvironmentSettingValue.Custom
	) {
		const customCloud = authProviderConfig.get<
			azureEnv.EnvironmentParameters | undefined
		...
		throw new Error(
			vscode.l10n.t(
				"The custom cloud choice is not configured. Please configure the setting `{0}.{1}`.",
				CustomCloudConfigurationSection,
				CustomEnvironmentSettingName,
			),
		);
...
export async function setConfiguredAzureEnv(
	cloud:
		...
		| "USGovernment"
		| undefined
		| azureEnv.EnvironmentParameters,
...
export function getConfiguredAuthProviderId(): string {
	return getConfiguredAzureEnv().name === azureEnv.Environment.AzureCloud.name
		? "microsoft"
