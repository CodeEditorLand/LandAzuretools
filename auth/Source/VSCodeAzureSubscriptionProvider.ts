	...
	implements AzureSubscriptionProvider
{
	...
	public constructor(private readonly logger?: vscode.LogOutputChannel) {
		const disposable = vscode.authentication.onDidChangeSessions(
			async (e) => {
	...
	public async getTenants(
		...
		for await (account of account
			? [account]
			: await vscode.authentication.getAccounts(
					getConfiguredAuthProviderId(),
				)) {
		...
		this.logger?.debug(
			`auth: Got ${results.length} tenants for account "${account?.label}" in ${endTimeMs - startTimeMs}ms`,
		);
		return results;
	}
	...
	public async getSubscriptions(
			...
			for (const account of accounts) {
				for (const tenant of await this.getTenants(account)) {
					...
					allSubscriptions.push(
						...(await this.getSubscriptionsForTenant(
							account,
							tenantId,
						)),
					);
				...
				allSubscriptions.push(
					...(await this.getSubscriptionsForTenant(account)),
				);
			}
		} finally {
		...
		this.logger?.debug(
			`auth: Got ${uniqueSubscriptions.length} subscriptions from ${accountCount} accounts in ${endTime - startTime}ms`,
		);
		...
		if (filter && !!subscriptionIds.length) {
			...
			return sortSubscriptions(
				uniqueSubscriptions.filter((sub) =>
					subscriptionIds.includes(sub.subscriptionId),
				),
			);
		}
	...
	public async isSignedIn(
		...
		async function silentlyCheckForSession(
			tenantId?: string,
			account?: vscode.AuthenticationSessionAccountInformation,
		): Promise<boolean> {
			return !!(await getSessionFromVSCode([], tenantId, {
				createIfNone: false,
				silent: true,
				account,
			}));
		...
		this.logger?.trace(
			`auth: isSignedIn returned ${result} (account="${account?.label ?? "none"}") (tenantId="${tenantId ?? "none"}")`,
		);
		return result;
	}
	...
	public async signIn(
		...
		this.logger?.debug(
			`auth: Signing in (account="${account?.label ?? "none"}") (tenantId="${tenantId ?? "none"}")`,
		);
	...
	public signOut(): Promise<void> {
		throw new Error(
			vscode.l10n.t(
				"Signing out programmatically is not supported. You must sign out by selecting the account in the Accounts menu and choosing Sign Out.",
			),
		);
	}
	 ...
	 */
	private async getSubscriptionsForTenant(
		account: vscode.AuthenticationSessionAccountInformation,
		tenantId?: string,
	): Promise<AzureSubscription[]> {
		...
		return {
			...
			authentication: {
				...
				getSessionWithScopes: (scopes) => {
					return getSessionFromVSCode(scopes, tenantId, {
						createIfNone: false,
						silent: true,
						account,
					});
