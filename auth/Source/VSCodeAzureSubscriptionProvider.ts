...
import type {
	SubscriptionClient,
	TenantIdDescription,
} from "@azure/arm-subscriptions"; // Keep this as `import type` to avoid actually loading the package before necessary
	...
	implements AzureSubscriptionProvider
{
	...
	public async getTenants(): Promise<TenantIdDescription[]> {
		const listTenantsResponse = await fetch(
		...
		return (
			(await listTenantsResponse.json()) as {
	...
	public async getSubscriptions(
		...
		try {
			...
			for (const tenant of await this.getTenants()) {
				...
				for (const subscription of await this.getSubscriptionsForTenant(
					tenantId
				)) {
					// If filtering is enabled, and the current subscription is not in that list, then skip it
					if (
						shouldFilterSubscriptions &&
