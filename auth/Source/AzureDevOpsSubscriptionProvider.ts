	...
	implements AzureSubscriptionProvider
{
	private _tokenCredential:
		| (TokenCredential & { tenantId?: string })
		| undefined;
	/**
	 * The resource ID of the Azure DevOps federated service connection,
	 *   which can be found on the `resourceId` field of the URL at the address bar
	...
	public async signIn(): Promise<boolean> {
		this._tokenCredential = await getTokenCredential(
	...
	private async getSubscriptionClient(
		...
		const accessToken =
			(
				await this._tokenCredential?.getToken(
...
async function getTokenCredential(
	...
	if (!process.env.AGENT_BUILDDIRECTORY) {
		...
		throw new Error(
			`Cannot create DevOps federated service connection credential outside of an Azure DevOps pipeline.`,
		);
	} else {
		console.log(
			`Creating DevOps federated service connection credential for service connection..`,
		);
		...
		return new ClientAssertionCredential(
			...
			clientId,
			async () =>
				await requestOidcToken(oidcRequestUrl, systemAccessToken),
		);
	}
}
