...
suite("showQuickPick", () => {
	suite("createQuickPickItems", () => {
		suite("sorting", () => {
			...
			async function testSorting(options: {
				input: TestPick[];
				recentlyUsed?: string | undefined;
				suppressPersistence?: boolean;
				expected: string[];
			}): Promise<void> {
				const input: types.IAzureQuickPickItem<string>[] =
					options.input.map(
						(p) =>
							<types.IAzureQuickPickItem<string>>{
								label: p.label,
								data: p.label,
				...
				if (options.recentlyUsed) {
					fakeState.fakeKeys[options.recentlyUsed] =
						await getrecentlyUsed(options.recentlyUsed);
				}

				// Execute
				const sortedPicks = await createQuickPickItems<
					types.IAzureQuickPickItem<string>
				>(
					input,
					{
			...
			test("Recently used should not have description changed if item's persistence suppressed", async () => {
				await testSorting({
					recentlyUsed: "p1",
					input: [{ label: "p1", suppressPersistance: true }],
					expected: ["p1"],
				});
			});

			suite(
				"Mix of priorities with recently used - high priority should be grouped together, order not changed within priority groups",
				() => {
