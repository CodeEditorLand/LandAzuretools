...
} from "../src/callWithTelemetryAndErrorHandling";
...
suite("callWithTelemetryAndErrorHandling tests", () => {
	test("sync", async () => {
		...
		assert.strictEqual(
			callWithTelemetryAndErrorHandlingSync("callbackId", testFuncError),
			undefined
		);
	...
	test("async", async () => {
		assert.strictEqual(
			await callWithTelemetryAndErrorHandling("callbackId", testFunc),
			testFunc()
		);
		assert.strictEqual(
	...
	test("Unexpected error during masking", async () => {
		...
		assert.strictEqual(
			await callWithTelemetryAndErrorHandling("callbackId", func),
			undefined
		);
	});
