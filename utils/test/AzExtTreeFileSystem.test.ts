...
class MockAzExtTreeFileSystem extends AzExtTreeFileSystem<MockItemType> {
	...
	public override async statImpl(
	...
	public override async readFileImpl(
		_context: IActionContext,
		_item: MockItemType,
		_originalUri
	): Promise<Uint8Array> {
	...
	public override async writeFileImpl(
		_context: IActionContext,
		_item: MockItemType
	) {
		return;
...
suite("AzExtTreeFileSystem", function () {
	...
	test("lookup doesn't find item not shown", async function () {
		...
		const uri: vscode.Uri = vscode.Uri.parse(
			`${mockAzExtTreeFileSystem.scheme}://mock_file_path?id=${id}`
		);
		try {
	...
	test("lookup finds shown item whose id is a valid query parameter", async function () {
		...
		const item = await mockAzExtTreeFileSystem.lookupPublic(
			mockContext,
			uri
		);
