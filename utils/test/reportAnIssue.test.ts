...
suite("getReportAnIssueLink", () => {
	const pasteIntoWindowBody: string =
		"https://github.com/Microsoft/azureextensionui/issues/new?body=The%20issue%20text%20was%20copied%20to%20the%20clipboard.%20%20Please%20paste%20it%20into%20this%20window.";
	...
	test("Short stack, entire body should be in link", async () => {
		...
		const link: string = await getReportAnIssueLink({
			callbackId: "actionId",
			time: Date.now(),
			error,
			issueProperties: {},
	...
	suite("issueProperties", () => {
		test("single property", async () => {
			...
			const link: string = await getReportAnIssueLink({
				...
				time: Date.now(),
				error,
				issueProperties: { property1: "Property #1" },
			});
