...
abstract class QuickPickStepBase extends AzureWizardPromptStep<ITestWizardContext> {
	...
	public async prompt(wizardContext: ITestWizardContext): Promise<void> {
		wizardContext[this.key] = (
			await wizardContext.ui.showQuickPick(
				[{ label: "Pick 1" }, { label: "Pick 2" }, { label: "Pick 3" }],
				{}
			)
		).label;
	}
...
class InputBoxStepIfNotPick1 extends AzureWizardPromptStep<ITestWizardContext> {
	...
	public shouldPrompt(wizardContext: ITestWizardContext): boolean {
		return (
			!wizardContext[this._key] &&
			wizardContext[quickPick1Key] !== "Pick 1"
		);
	}
}
...
class SubExecuteStep extends AzureWizardExecuteStep<ITestWizardContext> {
	...
	public async execute(wizardContext: ITestWizardContext): Promise<void> {
		if (wizardContext[executeKey1] || wizardContext[executeKey2]) {
			assert.fail(
				"SubExecuteStep should be executed before ExecuteStep1 or ExecuteStep2"
			);
		}
...
class SubSubExecuteStep extends AzureWizardExecuteStep<ITestWizardContext> {
	...
	public async execute(wizardContext: ITestWizardContext): Promise<void> {
		if (
			wizardContext[executeKey1] ||
			wizardContext[executeKey2] ||
			wizardContext[subExecuteKey]
		) {
			assert.fail(
				"SubSubExecuteStep should be executed before ExecuteStep1, ExecuteStep2, and SubExecuteStep"
			);
		}
...
abstract class QuickPickStepWithSubWizardBase extends QuickPickStepBase {
	...
	public async getSubWizard(
		wizardContext: ITestWizardContext
	): Promise<types.IWizardOptions<ITestWizardContext> | undefined> {
...
class QuickPickStepWithSubSubExecute extends QuickPickStepWithSubWizardBase {
	...
	protected getSubWizardInternal(): types.IWizardOptions<ITestWizardContext> {
		return {
			promptSteps: [
				new QuickPickStepWithSubWizard(new SubSubExecuteStep()),
			],
...
class QuickPickStepSubWizardNoExecute extends AzureWizardPromptStep<ITestWizardContext> {
	...
	public async getSubWizard(
		_wizardContext: ITestWizardContext
	): Promise<types.IWizardOptions<ITestWizardContext> | undefined> {
		return {
			promptSteps: [new SubInputBoxStep()],
		};
...
class StepWithSubWizardAndNoPrompt extends AzureWizardPromptStep<ITestWizardContext> {
	...
	public async getSubWizard(): Promise<
		types.IWizardOptions<ITestWizardContext>
	> {
		return {
			executeSteps: [new SubExecuteStep()],
		};
...
async function validateWizard(
	options: types.IWizardOptions<ITestWizardContext>,
	inputs: (string | TestInput)[],
	expectedContext: Partial<ITestWizardContext>
): Promise<void> {
	const testUserInput: TestUserInput = new TestUserInput(vscode);
	const context: ITestWizardContext = {
	...
	const wizard: AzureWizard<ITestWizardContext> = new AzureWizard(
		context,
		options
	);
...
suite("AzureWizard tests", () => {
	...
	test("InputBox back", async () => {
		await validateWizard(
			...
			[
				"testValue",
				TestInput.BackButton,
				"testValueChanged",
				"testValue3",
			],
			{
				inputBox1: "testValueChanged",
				inputBox2: "testValue3",
				execute1: "executeValue1",
			}
		);
	});
		...
		await validateWizard(
			{
				promptSteps: [
					new InputBoxStep1(),
					new QuickPickStepWithSubWizard(),
				],
				executeSteps: [new ExecuteStep1()],
			},
			...
			{
				inputBox1: "testValue",
				subInputBox: "testValue2",
				execute1: "executeValue1",
				subExecute: "subExecuteValue",
			}
		);
	});
		...
		await validateWizard(
			{
				promptSteps: [
					new QuickPickStepWithSubWizard(),
					new QuickPickStepWithMultiStepSubWizard(),
				],
				executeSteps: [],
			},
			["Create", "testValue1", "Create", "testValue2", "Pick 1"],
			{
				subInputBox: "testValue1",
				inputBox2: "testValue2",
				quickPick2: "Pick 1",
				execute1: "executeValue1",
				execute2: "executeValue2",
		...
		await validateWizard(
			{
				promptSteps: [
					new InputBoxStep1(),
					new QuickPickStepWithSubSubWizard(),
					new InputBoxStep2(),
				],
				executeSteps: [new ExecuteStep1()],
			},
	...
	test("Sub sub wizard execute in order", async () => {
		await validateWizard(
			...
			["Create", "Create", "testValue1"],
			{
				...
				execute1: "executeValue1",
				subExecute: "subExecuteValue",
				subSubExecute: "subSubExecuteValue",
			}
		);
	});
	...
	test("Back button into sub wizard", async () => {
		await validateWizard(
			...
			{
				inputBox1: "testValue",
				subInputBox: "subTestValueChanged",
				quickPick1: "Pick 1",
				execute1: "executeValue1",
				subExecute: "subExecuteValue",
			}
		...
		await validateWizard(
			{
				promptSteps: [
					new InputBoxStep1(),
					new QuickPickStepSubWizardNoExecute(),
				],
				executeSteps: [new ExecuteStep1()],
			},
			[
		...
		await validateWizard(
			{
				promptSteps: [
					new QuickPickStep1(),
					new StepWithSubWizardAndNoPrompt(),
				],
				executeSteps: [new ExecuteStep1()],
			},
			["Pick 1"],
		...
		await validateWizard(
			{
				promptSteps: [
					new QuickPickStep1(),
					new QuickPickStepAndNoPromptWithSubWizardOnlyIfPrevIsPick1(),
					new QuickPickStep2(),
				],
				executeSteps: [new ExecuteStep1()],
			},
			...
			],
			{
				...
				quickPick2: "Pick 1",
				execute1: "executeValue1",
				subQuickPick1: undefined,
				subQuickPick2: undefined,
			}
		);
	});
});
