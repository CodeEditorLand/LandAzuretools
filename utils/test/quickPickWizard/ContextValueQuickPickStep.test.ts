...
suite("ContextValueQuickPickStep tests", () => {
	...
	suite("Respect skipIfOne setting", () => {
		...
		test("Skip step with single pick when skipIfOne is true", async () => {
			...
			const pick = await runWizardWithStep(
				step,
...
async function runWizardWithStep(
	step: ContextValueQuickPickStep<
		QuickPickWizardContext,
		ContextValueFilterQuickPickOptions
	>,
	context: IActionContext
): Promise<TestTreeNode> {
	const wizardContext: QuickPickWizardContext = {
		...context,
		pickedNodes: [],
	};
