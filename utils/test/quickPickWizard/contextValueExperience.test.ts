...
suite("contextValueExperience", () => {
			...
			children: [
				{
					...
					contextValue: createContextValue(
						functionAppsGroupContextValues
					),
					children: [
						{
							id: azureFunction1,
	...
	test("Pick nested tree item", async () => {
		await runWithTestActionContext(
			"contextValueExperienceTest",
			async (context: TestActionContext) => {
				const tdp = createTestTreeDataProvider(treeWithContextValues());
				await context.ui.runWithInputs(
					[
						subscription1Label,
						functionAppsGroupLabel,
						azureFunction1Label,
					],
					async () => {
						const pick = await contextValueExperience<TestTreeNode>(
							context,
							tdp,
							{
				...
				await context.ui.runWithInputs(
					...
					async () => {
						await assertNoMatchingQuickPickItem(async () => {
							await contextValueExperience<TestTreeNode>(
								context,
								tdp,
								{
									include: azureFunctionContextValues,
	...
	test("Pick a tree item that has children", async () => {
		await runWithTestActionContext(
			...
			async (context: TestActionContext) => {
				const tdp = createTestTreeDataProvider(treeWithContextValues());
				await context.ui.runWithInputs(
					[subscription1Label, functionAppsGroupLabel],
					async () => {
						const pick = await contextValueExperience<TestTreeNode>(
							context,
							tdp,
