...
suite("masking", () => {
	...
	const credentialsSpecialChars: string =
		"sc()HQ*E+RrAlXSm[CeN1$$$rhDz^^HWeDz2X[t5R343HgCN.xS0x]swc|A2CÑwf¬da";
	const credentialsWithReservedChars: string =
		"$scHQERrAlXSmlCeN1mrhDzsHWeDz2XZt5R343HgCNmxS0xlswcaA2Cowflda";
	...
	suite("maskUserInfo", () => {
		...
		test("Multiple action and extension values", async () => {
			...
			assert.strictEqual(
				maskUserInfo(
					`${extensionValue1} ${extensionValue2} action1 action2`,
					["action1", "action2"]
				),
				"--- --- --- ---"
			);
		...
		test("One value is substring of another", async () => {
			assert.strictEqual(
				maskUserInfo("firstNameLastName", [
					"firstName",
					"firstNameLastName",
				]),
				"---"
		...
		test("Values from azure id", async () => {
			...
			assert.strictEqual(
				maskUserInfo(appId, context.valuesToMask),
		...
		test("Email", async () => {
			assert.strictEqual(
				maskUserInfo(
					"user@microsoft.com user2@microsoft.com us---Er@mic.rosoft.com",
					[]
				),
		...
		test("Username", async () => {
			...
			assert.strictEqual(
				maskUserInfo(
					"User dave cannot do that",
					[],
					false,
					getUsername
				),
				"User redacted:username cannot do that"
			);
			assert.strictEqual(
				maskUserInfo("User davecan do that", [], false, getUsername),
				"User davecan do that"
			);
			assert.strictEqual(
				maskUserInfo("Userdave can do that", [], false, getUsername),
				"Userdave can do that"
			);
			assert.strictEqual(
				maskUserInfo(
					"User/dave cannot do that",
			...
			assert.strictEqual(
				maskUserInfo("Dave cannot do that", [], false, getUsername),
				"redacted:username cannot do that"
			);
			assert.strictEqual(
				maskUserInfo("Cannot do that, Dave", [], false, getUsername),
				"Cannot do that, redacted:username"
			);
		});
		...
		test("Short Username", async () => {
			...
			assert.strictEqual(
				maskUserInfo("User dav can do that", [], false, getUsername),
				"User dav can do that"
			);
			assert.strictEqual(
				maskUserInfo("Userdav can do that", [], false, getUsername),
				"Userdav can do that"
			);
			assert.strictEqual(
				maskUserInfo("Davuser can do that", [], false, getUsername),
				"Davuser can do that"
			);
		});
		...
		test("Guid", async () => {
			assert.strictEqual(
				maskUserInfo(
					"c35d6342-5917-46f8-953e-9d3faffd1c72 C35D6342-5917-46F8-953E-9D3FAFFD1C72 c35d6342591746f8953e9d3faffd1c72",
					[]
				),
				"redacted:id redacted:id redacted:id"
			);
		});

		test("id with word breaks", async () => {
			assert.strictEqual(
				maskUserInfo("(c35d6342) \"c35d6342\" 'c35d6342'", []),
				"(redacted:id) \"redacted:id\" 'redacted:id'"
			);
		});
		...
		test("Ip address", async () => {
			assert.strictEqual(
				maskUserInfo(
					"127.0.0.1 aaaa:0000:0000:0000:ffff:0000:0000:0001",
					[]
				),
		...
		test("Phone number", async () => {
			assert.strictEqual(
				maskUserInfo("000-0000 000-111-2222", []),
				"redacted:id redacted:id"
			);
		});
		...
		test("Url", async () => {
			assert.strictEqual(
				maskUserInfo(
					"https://microsoft.com http://microsoft.com mongodb://mongodb0.example.com:27017",
					[]
				),
		...
		test("Url without scheme", async () => {
			assert.strictEqual(
				maskUserInfo(
					"microsoft.com microsoft.org?queryParam=test1 microsoft.net",
					[]
				),
				"redacted:url redacted:url redacted:url"
			);
		...
		test("Url masking exclusions", async () => {
			...
			assert.strictEqual(
				maskUserInfo(
					"microsoft.NET .NET ASP.NET microsoft-asp.net microsoftasp.net",
					[]
				),
				"redacted:url .NET ASP.NET redacted:url redacted:url"
			);
			assert.strictEqual(
				maskUserInfo(
					"http://microsoft.NET http://.NET http://ASP.NET http://microsoft-asp.net http://microsoftasp.net",
					[]
				),
				"redacted:url redacted:url redacted:url redacted:url redacted:url"
			);
		});
		...
		test("key", async () => {
			assert.strictEqual(
				maskUserInfo(
					"sv=2012-02-12&st=2009-02-09&se=2009-02-10&sr=c&sp=r&si=YWJjZGVmZw%3d%3d&sig=dddddddddddddddddddddddddddddddddddddddddddddddd",
					[]
				),
				"redacted:key"
			);
			assert.strictEqual(
				maskUserInfo(
					"DefaultEndpointsProtocol=http;AccountName=devstoreaccount1;AccountKey=dddddddddddddddddddddddddddddddddddddddddddddddd/dddddddd/dddddddd==;",
					[]
				),
				"redacted:key"
			);
			assert.strictEqual(
				maskUserInfo(
					"AccountEndpoint=accountname.documents.azure:443;AccountKey=accountkey==;",
					[]
				),
				"redacted:key"
			);
			assert.strictEqual(
				maskUserInfo("microsoft.co?token=dddd", []),
				"redacted:key"
			);
			assert.strictEqual(
				maskUserInfo("--password ddddddd", []),
				"redacted:key"
			);
			assert.strictEqual(
				maskUserInfo('"POSTGRES_PASSWORD": "ddddddd"', []),
				"redacted:key"
			);
			assert.strictEqual(
				maskUserInfo("PGPASSWORD=ddddddd", []),
				"redacted:key"
			);
			assert.strictEqual(
				maskUserInfo('pwd: "ddddddd!@#$%^&*()_+"', []),
				"redacted:key"
			);
		});
		...
		test("lessAggressive", async () => {
			...
			assert.strictEqual(
				maskUserInfo(
					"https://microsoft.com c35d6342-5917-46f8-953e-9d3faffd1c72 hello@world accountkey=1234 valueToMask",
					[valueToMask],
					true /* lessAggressive */
				),
				"redacted:url c35d6342-5917-46f8-953e-9d3faffd1c72 hello@world redacted:key ---"
			);
		});
...
function validateError(err: unknown, value: string): boolean {
	return (
		!JSON.stringify(parseError(err)).includes(value) &&
