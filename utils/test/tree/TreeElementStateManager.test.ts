...
suite("TreeElementStateManager", () => {
	...
	test("wrapItemInStateHandling - getChildren returns unmodified children", async () => {
		...
		const wrappedItem = state.wrapItemInStateHandling(
			testItemWithChildren,
			async () => {}
		);

		const originalChildren = await (
			testItemWithChildren as TreeElementBase
		).getChildren?.();
	...
	test("runWithTemporaryDescription - behaves as expected", async () => {
		...
		const originalDescription = (await wrappedItem.getTreeItem())
			.description;
		assert.strictEqual(
			originalDescription,
			testDescription,
			"TreeItem.description should be the original description."
		);

		const temporaryDescription = "temporary description";
		let updatedDescription: string | boolean | undefined;
		await state.runWithTemporaryDescription(
			wrappedItem.id,
			temporaryDescription,
			async () => {
				updatedDescription = (await wrappedItem.getTreeItem())
					.description;
			}
		);
		...
		assert.strictEqual(
			updatedDescription,
			temporaryDescription,
			"TreeItem.description should be the temporary description while the callback is running."
		);

		const descriptionAfterCallback = (await wrappedItem.getTreeItem())
			.description;
		assert.strictEqual(
			descriptionAfterCallback,
			originalDescription,
			"TreeItem.description should be the original description after the callback is done."
		);
	});
		...
		await state.runWithTemporaryDescription(
			...
			async () => {
				...
				assert.strictEqual(
					updatedDescription,
					temporaryDescription,
					"TreeItem.description should be the temporary description"
				);
			},
			true
		);
	...
	test("showCreatingChild - behaves as expected", async () => {
		...
		await state.showCreatingChild(
			wrappedItem.id,
			"Creating...",
			async () => {
				// do nothing

				const childrenDuringCallback =
					(await wrappedItem.getChildren?.()) ?? [];
				assert.strictEqual(
					childrenDuringCallback.length,
				...
				const creatingTreeItem = treeItems.find(
					(ti) => ti.label === "Creating..."
				);
		...
		assert.strictEqual(
			creatingTreeItem,
			undefined,
			"Creating... tree item should be removed after the callback is done."
		);
	});
});
