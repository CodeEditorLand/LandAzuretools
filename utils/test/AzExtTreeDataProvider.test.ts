...
suite("AzExtTreeDataProvider", () => {
	...
	test("getCachedChildren", async () => {
		...
		const middle: MiddleTreeItem[] = <MiddleTreeItem[]>(
			await root.getCachedChildren(context)
		);
	...
	test("loadAllChildren", async () => {
		...
		const middle: MiddleTreeItem[] = <MiddleTreeItem[]>(
			await root.loadAllChildren(context)
		);
		assert.equal(middle.length, 10);
		assert.equal(root.hasMoreChildrenImpl(), false);

		const first: MiddleTreeItem = middle[0];
		const leaves: AzExtTreeItem[] = <MiddleTreeItem[]>(
			await first.loadAllChildren(context)
		);
	...
	const findTestCases: IFindTestCase[] = [
		...
		},
		{
			...
			loadAll: true,
			expectedLabel: "5",
			expectedLeafCounts: [2, 2, 6, 2],
		},
		{
			id: "/none",
			loadAll: true,
			expectedLabel: undefined,
			expectedLeafCounts: [2, 2, 2, 2, 2, 2, 2, 2, 2, 2],
		},
		{
			id: "/10",
			loadAll: true,
			expectedLabel: "10",
			expectedLeafCounts: [2, 2, 2, 2, 2, 2, 2, 2, 2, 2],
		}, // https://github.com/Microsoft/vscode-cosmosdb/issues/488
	];
	...
	function addFindTestCase(testCase: IFindTestCase): void {
		...
		test(name, async () => {
			...
			const result: AzExtTreeItem | undefined = await tree.findTreeItem(
				testCase.id,
				{ ...context, loadAll: testCase.loadAll }
			);
			assert.equal(result && result.label, testCase.expectedLabel);

			const middles: MiddleTreeItem[] = <MiddleTreeItem[]>(
				await root.getCachedChildren(context)
			);
			assert.equal(middles.length, testCase.expectedLeafCounts.length);

			const leafCounts: number[] = await Promise.all(
