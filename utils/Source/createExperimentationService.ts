...
export async function createExperimentationService(
	...
	targetPopulation?: tas.TargetPopulation,
	...
	if (targetPopulation === undefined) {
		...
		} else if (/alpha/gi.test(extensionVersion)) {
			// Extension version has "alpha"
			targetPopulation = tas.TargetPopulation.Insiders;
		} else if (/Insiders/gi.test(vscode.env.appName)) {
	...
	try {
		result.wrappedExperimentationService =
			await tas.getExperimentationServiceAsync(
				extensionId,
				extensionVersion,
				targetPopulation,
...
class ExperimentationServiceAdapter implements IExperimentationServiceAdapter {
	...
	public async getCachedTreatmentVariable<
		T extends string | number | boolean,
	>(name: string): Promise<T | undefined> {
		if (!this.wrappedExperimentationService) {
			return Promise.resolve(undefined);
		}

		return Promise.resolve(
			this.wrappedExperimentationService.getTreatmentVariable<T>(
				"vscode",
				name,
			),
		);
	...
	public async getLiveTreatmentVariable<T extends string | number | boolean>(
		name: string,
	): Promise<T | undefined> {
		if (!this.wrappedExperimentationService) {
			return undefined;
		...
		return this.wrappedExperimentationService.getTreatmentVariableAsync(
			"vscode",
			name,
		);
	}
...
class ExperimentationTelemetry implements tas.IExperimentationTelemetry {
	...
	public constructor(
		private readonly telemetryReporter: IInternalTelemetryReporter,
		context: vscode.ExtensionContext,
	) {
		context.subscriptions.push(
			registerTelemetryHandler((actionContext: IActionContext) =>
				this.handleTelemetry(actionContext),
			),
		);
	}
