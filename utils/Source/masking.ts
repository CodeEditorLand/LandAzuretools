...
export function addExtensionValueToMask(
 ...
 */
export function addValuesToMaskFromAzureId(
...
export async function callWithMaskHandling<T>(
	callback: () => Promise<T>,
	valueToMask: string,
): Promise<T> {
	try {
...
export function maskUserInfo(
	unknownArg: unknown,
	actionValuesToMask: string[],
	lessAggressive: boolean = false,
