...
import {
	...
	IInternalTelemetryReporter,
} from "./createTelemetryReporter";
...
class UninitializedExtensionVariables implements types.UIExtensionVariables {
	private _error: Error = new Error(
		l10n.t(
			'"registerUIExtensionVariables" must be called before using the vscode-azureextensionui package.',
		),
	);
 ...
 */
export let ext: IInternalExtensionVariables =
	new UninitializedExtensionVariables();

export function registerUIExtensionVariables(
	extVars: types.UIExtensionVariables,
): void {
	if (ext === extVars) {
		// already registered
		return;
	...
	ext = Object.assign(extVars, {
		_internalReporter: createTelemetryReporter(extVars.context),
	});
 ...
 */
async function handleEntryNotFound(
	context: types.IErrorHandlerContext,
): Promise<void> {
