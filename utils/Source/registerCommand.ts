...
function isTreeElementBase(object?: unknown): object is types.TreeElementBase {
	return (
		typeof object === "object" && object !== null && "getTreeItem" in object
	);
}
	...
	ext.context.subscriptions.push(
		commands.registerCommand(
			...
			async (...args: unknown[]): Promise<unknown> => {
				if (debounce) {
					/* Only check for debounce if registered command specifies */
					...
					async (context: types.IActionContext) => {
						if (args.length > 0) {
							...
							if (firstArg instanceof Uri) {
								...
								"contextValue" in firstArg &&
								typeof firstArg.contextValue === "string"
							) {
								context.telemetry.properties.contextValue =
									firstArg.contextValue;
							} else if (isTreeElementBase(firstArg)) {
								context.telemetry.properties.contextValue = (
									await firstArg.getTreeItem()
								).contextValue;
							}
