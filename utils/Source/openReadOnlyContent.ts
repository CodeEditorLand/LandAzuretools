...
export async function stashReadOnlyContent(
	node: { label: string; fullId: string },
	content: string,
...
export function stashReadOnlyContentSync(
	node: { label: string },
	content: string,
	fileExtension: string,
): ReadOnlyContent {
...
class ReadOnlyContentProvider implements TextDocumentContentProvider {
	...
	private stashReadOnlyContentCore(
		label: string,
		fileId: string,
		fileExtension: string,
		content: string,
	...
	public async stashReadOnlyContent(
		node: { label: string; fullId: string },
		content: string,
		fileExtension: string,
	): Promise<ReadOnlyContent> {
	...
	public stashReadOnlyContentSync(
		node: { label: string },
		content: string,
		fileExtension: string,
	): ReadOnlyContent {
	...
	public async openReadOnlyContent(
		...
		const readOnlyContent = await this.stashReadOnlyContent(
			node,
			content,
			fileExtension,
		);
