...
> extends ContextValueQuickPickStep<TContext, TOptions> {
	...
	protected override pickFilter: PickFilter<TreeItem> =
		new CompatibleContextValuePickFilter(this.pickOptions);
	...
	private setCustomPlaceholder(context: TContext): void {
		...
		if (
			...
			this.promptOptions.placeHolder = l10n.t(
				"Select {0}",
				lastPickedItemUnwrapped.childTypeLabel
			);
		}
	 ...
	 */
	private async provideCompatabilityWithPickTreeItemImpl(
	...
	private async getCustomChildren(
		context: TContext,
		node: AzExtParentTreeItem
	): Promise<AzExtTreeItem | undefined> {
		return await node.pickTreeItemImpl?.(
			Array.isArray(this.pickOptions.contextValueFilter.include)
