...
interface CompatibilityRecursiveQuickPickOptions
	extends types.ContextValueFilterQuickPickOptions {
 ...
 */
export class CompatibilityRecursiveQuickPickStep<
	TContext extends types.QuickPickWizardContext &
		types.ITreeItemPickerContext,
> extends CompatibilityContextValueQuickPickStep<
	TContext,
	CompatibilityRecursiveQuickPickOptions
> {
	protected override async promptInternal(
		...
		if (isAzExtParentTreeItem(lastPickedItemTi)) {
			...
			this.promptOptions.placeHolder = vscode.l10n.t(
				"Select {0}",
		...
		const shouldAddCreatePick =
			isAzExtParentTreeItem(lastPickedItemTi) &&
			!!lastPickedItemTi.createChildImpl &&
			!!lastPickedItemTi.childTypeLabel &&
			!wizardContext.suppressCreatePick;

		this.pickOptions.create = shouldAddCreatePick
			? {
					callback: lastPickedItemTi.createChild.bind(
						lastPickedItemTi,
					) as typeof lastPickedItemTi.createChild,
					// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
					label:
						lastPickedItemTi.createNewLabel ??
						vscode.l10n.t(
							"$(plus) Create new {0}...",
							lastPickedItemTi.childTypeLabel!,
						),
		...
		if (
			...
			if (typeof selected.data === "function") {
				...
				const callback =
					selected.data as unknown as CreateCallback<unknown>;
				...
				const createdTreeItem = (await callback?.(
					wizardContext,
				)) as AzExtTreeItem;
	...
	public async getSubWizard(
		...
		const ti = isAzExtTreeItem(lastPickedItem)
			? lastPickedItem
			: ((await this.treeDataProvider.getTreeItem(
					lastPickedItem,
				)) as AzExtTreeItem);
		...
		if (this.pickFilter.isFinalPick(ti, lastPickedItem)) {
			...
			return {
				hideStepCount: true,
				promptSteps: [
					new CompatibilityRecursiveQuickPickStep(
	...
	protected override async getPicks(
		wizardContext: TContext,
		...
		try {
			...
			if (
				error instanceof NoResourceFoundError &&
				!!this.pickOptions.create
			) {
	...
	private getCreatePick(
		options: CreateOptions,
	): types.IAzureQuickPickItem<CreateCallback> {
		return {
