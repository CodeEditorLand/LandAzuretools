...
export class QuickPickAzureSubscriptionStep extends GenericQuickPickStepWithCommands<
	AzureResourceQuickPickWizardContext,
	SkipIfOneQuickPickOptions
> {
	...
	public constructor(
		...
		options?: AzureSubscriptionQuickPickOptions,
	) {
		super(
			tdp,
			{
		...
		this.pickFilter = new AzureSubscriptionPickFilter(
			options?.selectBySubscriptionId,
		);
	}
	...
	protected override async promptInternal(
		...
		const pickedSubscription = (await super.promptInternal(
			wizardContext,
		)) as SubscriptionItem;
...
class AzureSubscriptionPickFilter implements PickFilter {
	...
	isAncestorPick(
		...
		element: SubscriptionItem,
	): boolean {
		// All nodes at this level are always subscription nodes
		return this.subscriptionId
			? element.subscription.subscriptionId === this.subscriptionId
			: true;
	}
}
