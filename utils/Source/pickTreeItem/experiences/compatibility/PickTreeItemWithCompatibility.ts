...
export namespace PickTreeItemWithCompatibility {
	 ...
	 */
	export async function resource<TPick extends types.AzExtTreeItem>(
	...
	export async function subscription(
		...
		const applicationSubscription = await subscriptionExperience(
			context,
			tdp,
		);
	 ...
	 */
	export async function showTreeItemPicker<TPick extends types.AzExtTreeItem>(
		...
		const promptSteps: AzureWizardPromptStep<
			types.QuickPickWizardContext & types.ITreeItemPickerContext
		>[] = [
		...
		const wizardContext: types.QuickPickWizardContext &
		...
		if (!lastPickedItem) {
			...
			return isWrapper(lastPickedItem)
				? lastPickedItem.unwrap<TPick>()
