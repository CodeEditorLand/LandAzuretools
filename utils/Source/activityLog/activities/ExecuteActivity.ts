...
> extends ActivityBase<void> {
	...
	public progressState(): hTypes.ActivityTreeItemOptions {
		return {
			label: this.label,
			getChildren: this.context.activityChildren
				? (parent: AzExtParentTreeItem) => {
						parent.compareChildrenImpl = () => 0; // Don't sort
						return this.context.activityChildren || [];
					}
	...
	private appendErrorItemToActivityChildren(errorItem: AzExtTreeItem): void {
		...
		if (
			(this.context as unknown as types.IActionContext).errorHandling
				?.suppressDisplay
		) {
			return;
		}
		...
		if (
			...
			new RegExp(activityFailContext).test(
				lastActivityChild?.contextValue ?? "",
			)
		) {
			const previousLoadMoreChildrenImpl =
			...
			lastActivityChild.loadMoreChildrenImpl = async (
				...
				return [
					...(await previousLoadMoreChildrenImpl(
						clearCache,
						context,
					)),
					errorItem,
