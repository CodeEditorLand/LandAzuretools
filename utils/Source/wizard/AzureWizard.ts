	...
	public async prompt(): Promise<void> {
		...
		try {
			...
			while (step) {
				...
				this._context.telemetry.properties.lastStep = `prompt-${getEffectiveStepId(
					step
				)}`;
			...
			async (progress) => {
				...
				while (step) {
					if (step.shouldExecute(this._context)) {
						this._context.telemetry.properties.lastStep = `execute-${getEffectiveStepId(
							step
						)}`;
	...
	private addSubWizard(
		...
		if (subWizard.promptSteps) {
			...
			this._promptSteps.push(
				...(<AzureWizardPromptStep<T>[]>subWizard.promptSteps.reverse())
			);
