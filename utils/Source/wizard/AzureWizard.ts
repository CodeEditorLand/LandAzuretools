...
import {
	...
	IInternalAzureWizard,
} from "../userInput/IInternalActionContext";
...
export class AzureWizard<
		T extends IInternalActionContext &
			Partial<types.ExecuteActivityContext>,
	>
	implements types.AzureWizard<T>, IInternalAzureWizard
{
	...
	public getCachedInputBoxValue(): string | undefined {
		return this.currentStepId
			? this._cachedInputBoxValues[this.currentStepId]
			: undefined;
	}
	...
	public get totalSteps(): number {
		return (
			this._finishedPromptSteps.filter((s) => s.prompted).length +
			this._promptSteps.filter((s) => s.shouldPrompt(this._context))
				.length +
			1
		);
	}
	...
	public async prompt(): Promise<void> {
		...
		try {
			let step: AzureWizardPromptStep<T> | undefined =
				this._promptSteps.pop();

			while (step) {
				if (
					this._context.ui.wizard?.cancellationToken
				...
				step.propertiesBeforePrompt = Object.keys(this._context).filter(
					(k) => !isNullOrUndefined(this._context[k]),
				);
				...
				if (step.shouldPrompt(this._context)) {
					const loadingQuickPick = this._showLoadingPrompt
						? createQuickPick(this._context, {
					...
					if (loadingQuickPick) {
						disposables.push(
							loadingQuickPick?.onDidHide(() => {
								...
								if (
									!this._context.ui.isPrompting &&
									!this._context.ui.isTesting
								) {
					...
					try {
						...
						if (pe.errorType === "GoBackError") {
							// Use `errorType` instead of `instanceof` so that tests can also hit this case
		...
		await this.withProgress(
			...
			async (progress) => {
				...
				const steps: AzureWizardExecuteStep<T>[] =
					this._executeSteps.sort((a, b) => b.priority - a.priority);
				...
				while (step) {
					...
					const progressOutput:
						| types.ExecuteActivityOutput
						| undefined = step.createProgressOutput?.(
						this._context,
					);

					if (progressOutput) {
						this.displayActivityOutput(
							progressOutput,
							step.options,
						);
					}
					...
					try {
						...
						if (progressOutput?.item) {
							this._context.activityChildren =
								this._context.activityChildren?.filter(
									(t) => t !== progressOutput.item,
								);
						}
	...
	private displayActivityOutput(
		output: types.ExecuteActivityOutput,
		options: types.AzureWizardExecuteStepOptions,
	): void {
		if (
			output.item &&
	...
	private async withProgress(
		...
		if (this._context.registerActivity) {
			...
			const WizardActivity =
				this._context.wizardActivity ?? ExecuteActivity;
			...
			const activity = new WizardActivity(
				...
				async (activityProgress) => {
					if (this._context.suppressNotification) {
						await task(
							activityProgress,
							activity.cancellationTokenSource.token,
						);
					} else {
	...
	private goBack(
		currentStep: AzureWizardPromptStep<T>,
	...
	private addSubWizard(
		step: AzureWizardPromptStep<T>,
		subWizard: types.IWizardOptions<T>,
	): void {
		...
		if (subWizard.promptSteps) {
			subWizard.promptSteps = subWizard.promptSteps.filter((s1) => {
				return (
					s1.supportsDuplicateSteps ||
					!this._finishedPromptSteps
						.concat(this._promptSteps)
						.some(
							(s2) =>
								getEffectiveStepId(s1) ===
								getEffectiveStepId(s2),
						)
				);
			});
			...
			subWizard.promptSteps.forEach((s) => {
				(<AzureWizardPromptStep<T>>s).effectiveTitle =
					subWizard.title || step.effectiveTitle;
			});
		}
...
function getEffectiveStepId<T extends IInternalActionContext>(
	step: types.AzureWizardPromptStep<T> | types.AzureWizardExecuteStep<T>,
): string {
