...
export class GenericParentTreeItem
	extends AzExtParentTreeItem
	implements types.GenericParentTreeItem
{
	...
	constructor(
		...
		readonly options: GenericParentTreeItemOptions,
	) {
		super(parent);
		...
		this.initialCollapsibleState =
			options.initialCollapsibleState === undefined
				? TreeItemCollapsibleState.Collapsed
				: options.initialCollapsibleState;
	...
	public loadMoreChildrenImpl(
		...
		return (
			this.options.loadMoreChildrenImpl?.(clearCache, context) ??
			Promise.resolve([])
		);
	}
