...
export abstract class AzExtParentTreeItem
	extends AzExtTreeItem
	implements types.AzExtParentTreeItem, IAzExtParentTreeItemInternal
{
	...
	public async createChild<T extends types.AzExtTreeItem>(
		context: types.IActionContext & Partial<types.ICreateChildImplContext>,
	): Promise<T> {
		if (this.createChildImpl) {
			context.telemetry.properties.advancedCreation = String(
				!!context.advancedCreation,
			);
			...
			try {
				...
				if (creatingTreeItem) {
					this._creatingTreeItems.splice(
	...
	public async pickChildTreeItem(
		expectedContextValues: (string | RegExp)[],
		context: types.ITreeItemPickerContext,
	): Promise<AzExtTreeItem | AzExtTreeItem[]> {
		if (this.pickTreeItemImpl) {
			const children: AzExtTreeItem[] =
				await this.getCachedChildren(context);

			const pickedItem: AzExtTreeItem | undefined =
				await this.pickTreeItemImpl(expectedContextValues, context);

			if (pickedItem) {
				const child: AzExtTreeItem | undefined = children.find(
					(ti: AzExtTreeItem) => ti.fullId === pickedItem.fullId,
				);
		...
		try {
			getTreeItem = (
				await context.ui.showQuickPick(
					this.getQuickPicks(expectedContextValues, context),
					{
						placeHolder,
						stepName,
			...
			if (error instanceof AutoSelectError) {
				...
				const result: types.IAzureQuickPickItem<GetTreeItemFunction>[] =
					await context.ui.showQuickPick(error.picks, {
						placeHolder,
						stepName,
						canPickMany: true,
					});
				...
				return await Promise.all(
					result.map(async (pick) => await pick.data()),
				);
			} else {
				throw error;
			}
	...
	public addChildToCache(childToAdd: AzExtTreeItem): void {
		if (
			!this._cachedChildren.find((ti) => ti.fullId === childToAdd.fullId)
		) {
			...
			for (let i: number = 0; i < this._cachedChildren.length; i++) {
				if (
					childToAdd.label.localeCompare(
						this._cachedChildren[i].label,
					) < 1
				) {
					index = i;
		...
		await Promise.all(
			sourceArray.map(async (source: TSource) => {
				try {
					...
					if (name) {
						treeItems.push(
							new InvalidTreeItem(this, error, {
								label: name,
	...
	private async loadMoreChildrenInternal(
		context: types.IActionContext,
	): Promise<void> {
		try {
			if (this._clearCache) {
			...
			if (this.collapsibleState === TreeItemCollapsibleState.Expanded) {
				this.treeDataProvider.collapsibleStateTracker?.onDidExpandOrRefreshExpandedEmitter.fire(
					this,
				);
			}
	...
	private async getQuickPicks(
		...
		let autoSelectInTreeItemPicker: boolean | undefined =
			this.autoSelectInTreeItemPicker;

		const picks: types.IAzureQuickPickItem<GetTreeItemFunction>[] =
			children.map((ti: AzExtTreeItem) => {
					...
					return {
						...
						data: async (): Promise<AzExtTreeItem> => {
							if (!ti.commandId) {
								throw new Error(
									l10n.t(
										"Failed to find commandId on generic tree item.",
									),
								);
							} else {
		...
		if (
			this.createChildImpl &&
			this.childTypeLabel &&
			!context.suppressCreatePick
		) {
			const createNewLabel: string =
				this.createNewLabel ||
				l10n.t("Create new {0}...", this.childTypeLabel);
			...
			if (this.supportsAdvancedCreation) {
				picks.unshift({
					...
					data: async (): Promise<AzExtTreeItem> =>
						await this.createChild<AzExtTreeItem>(
							Object.assign(context, { advancedCreation: true }),
						),
				});
			}
			...
			picks.unshift({
				label: `$(plus) ${createNewLabel}`,
				data: async (): Promise<AzExtTreeItem> =>
					await this.createChild<AzExtTreeItem>(context),
			});
		}
		...
		if (picks.length === 0) {
			...
			context.canPickMany &&
			children.some((c) => c.matchesContextValue(expectedContextValues))
		) {
