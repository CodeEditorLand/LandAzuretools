...
export class TreeElementStateManager<
	TElement extends types.TreeElementWithId = types.TreeElementWithId,
> implements vscode.Disposable
{
	...
	async runWithTemporaryDescription<T = void>(
		...
		try {
			...
			this.update(
				...
				{
					...this.getState(id),
					temporaryDescription: undefined,
					spinner: false,
				},
				dontRefreshOnRemove,
			);
	...
	async showCreatingChild<T = void>(
		...
		return await this.runWithTemporaryChild(
			id,
	...
	private async runWithTemporaryChild<T = void>(
		id: string,
		child: types.TreeElementBase,
		callback: () => Promise<T>,
	): Promise<T> {
		this.update(id, {
			...this.getState(id),
			temporaryChildren: [
				child,
				...(this.getState(id).temporaryChildren ?? []),
			],
		});
		...
		try {
			...
			this.update(id, {
				...this.getState(id),
				temporaryChildren: this.getState(id).temporaryChildren?.filter(
	...
	private applyStateToTreeItem(
		state: Partial<types.TreeElementStateModel>,
	...
	private onDidRequestRefresh(id: string, callback: () => void): void {
		this.disposables.push(
			this.onDidUpdateStateEvent((eventId: string) => {
				if (eventId === id) {
					callback();
				}
			}),
