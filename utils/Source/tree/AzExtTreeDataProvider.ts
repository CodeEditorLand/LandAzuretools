...
export class AzExtTreeDataProvider
	implements
		IAzExtTreeDataProviderInternal,
		types.AzExtTreeDataProvider,
		Disposable
{
	...
	> = new Map<string, Promise<types.AzExtTreeItem | undefined>>();
	...
	public get onDidExpandOrRefreshExpandedTreeItem(): Event<AzExtTreeItem> {
		if (!this.collapsibleStateTracker) {
			throw new Error(
				"To use the `onDidExpandOrRefreshExpandedTreeItem`, first call `trackTreeItemCollapsibleState`.",
			);
		}

		return this.collapsibleStateTracker.onDidExpandOrRefreshExpandedEmitter
			.event;
	}
	...
	public trackTreeItemCollapsibleState(
		...
		return (this._collapsibleStateTracker = new CollapsibleStateTracker(
			treeView,
		));
	}
	...
	public async getChildren(
		arg?: AzExtParentTreeItem,
	): Promise<AzExtTreeItem[]> {
		try {
			return <AzExtTreeItem[]>(
					...
					async (context: types.IActionContext) => {
						...
						for (const child of children) {
							this.isDuplicateChild(child, resultMap)
								...
								: resultMap.set(
										child.fullIdWithContext || child.fullId,
										child,
									);
						}
						...
						if (hasMoreChildren && !treeItem.isLoadingMore) {
							const loadMoreTI: GenericTreeItem =
								new GenericTreeItem(treeItem, {
									label: loadMoreLabel,
	...
	public async getParent(
		...
		return treeItem.parent === this._rootTreeItem
			? undefined
			: treeItem.parent;
	}

	public async findTreeItem<T extends types.AzExtTreeItem>(
		fullId: string,
		context: types.IFindTreeItemContext,
	): Promise<T | undefined> {
		...
		if (existingTask) {
			...
			const newTask: Promise<types.AzExtTreeItem | undefined> =
				context.loadAll
					? runWithLoadingNotification(context, (cancellationToken) =>
							this.findTreeItemInternal(
								fullId,
								context,
								cancellationToken,
							),
	 ...
	 */
	private async findTreeItemInternal(
		fullId: string,
		context: types.IFindTreeItemContext,
	...
	private isDuplicateChild(
		child: AzExtTreeItem,
		children: Map<string, AzExtTreeItem>,
	): boolean {
...
function isAncestor(treeItem: AzExtTreeItem, fullId: string): boolean {
	...
	return (
		fullId.startsWith(`${treeItem.fullId}/`) &&
		isAzExtParentTreeItem(treeItem)
	);
}
