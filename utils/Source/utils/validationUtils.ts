...
export namespace validationUtils {
	...
	export function hasValidCharLength(
		...
		if (lowerLimitIncl < 1 || upperLimitIncl < 1) {
			throw new Error(
				vscode.l10n.t(
					"Specified character lengths should be 1 character or greater.",
				),
			);
		}
		...
		if (lowerLimitIncl > upperLimitIncl) {
			throw new Error(
				vscode.l10n.t(
					"The minimum specified character length should not exceed the maximum specified character length.",
				),
			);
		}
	...
	export function getInvalidCharLengthMessage(rc?: RangeConstraints): string {
		if (!rc?.lowerLimitIncl && !rc?.upperLimitIncl) {
			...
			return vscode.l10n.t(
				"The value must be {0} characters or greater.",
				rc.lowerLimitIncl,
			);
		} else if (!rc?.lowerLimitIncl && rc?.upperLimitIncl) {
			return vscode.l10n.t(
				"The value must be {0} characters or less.",
				rc.upperLimitIncl,
			);
		} else {
