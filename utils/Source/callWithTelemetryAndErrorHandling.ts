...
export function callWithTelemetryAndErrorHandlingSync<T>(
	callbackId: string,
	callback: (context: types.IActionContext) => T
): T | undefined {
...
export async function callWithTelemetryAndErrorHandling<T>(
	callbackId: string,
	callback: (context: types.IActionContext) => T | PromiseLike<T>
): Promise<T | undefined> {
	const [start, context] = initContext(callbackId);
...
export function registerOnActionStartHandler(
	handler: types.OnActionStartHandler
): Disposable {
...
export function registerTelemetryHandler(
	handler: types.TelemetryHandler
): Disposable {
...
function handleError(
	...
	try {
		...
		if (
			!context.errorHandling.suppressDisplay ||
		...
		if (!context.errorHandling.suppressDisplay) {
			...
			void window
				...
				.then(
					async (
						result: MessageItem | types.AzExtErrorButton | undefined
					) => {
...
function handleTelemetry(
	...
	try {
		...
		if (shouldSendTelemtry(context)) {
			...
			for (const [key, value] of Object.entries(
				...
				if (value) {
					if (/(error|exception)/i.test(key)) {
						context.telemetry.properties[key] = context.telemetry
							.maskEntireErrorMessage
							? getRedactedLabel("action")
							: maskUserInfo(value, context.valuesToMask);
					} else {
						context.telemetry.properties[key] = maskUserInfo(
							value,
							context.valuesToMask,
							true /* lessAggressive */
						);
			...
			ext._internalReporter.sendTelemetryErrorEvent(
				getTelemetryEventName(handlerContext),
				context.telemetry.properties,
...
function sendHandlerFailedEvent(
	context: types.IHandlerContext,
	handlerName: string
) {
	...
	try {
		if (shouldSendTelemtry(context)) {
			...
			ext._internalReporter.sendTelemetryErrorEvent(
				getTelemetryEventName(context),
				{ handlerFailed: handlerName }
			);
		}
	} catch {
...
function getTelemetryEventName(context: types.IHandlerContext): string {
	return context.telemetry.eventVersion
		? `${context.callbackId}V${context.telemetry.eventVersion}`
		: context.callbackId;
}
