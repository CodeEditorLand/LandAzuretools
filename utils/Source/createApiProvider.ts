...
function isAzureExtensionApiFactory(
	maybeAzureExtensionApiFactory: AzureExtensionApiFactory | AzureExtensionApi,
): maybeAzureExtensionApiFactory is AzureExtensionApiFactory {
	return (
		(<AzureExtensionApiFactory>maybeAzureExtensionApiFactory).createApi !==
		undefined
	);
}
...
export function createApiProvider(
	...
	for (const azExt of azExts) {
		if (!semver.valid(azExt.apiVersion)) {
			throw new Error(
				vscode.l10n.t('Invalid semver "{0}".', azExt.apiVersion),
			);
		}
	}
	...
	const apiFactories: AzureExtensionApiFactory[] = azExts.map(
		(azExt): AzureExtensionApiFactory => {
...
class ApiVersionError extends Error {
	constructor(
		message: string,
		readonly code: ApiVersionCode,
	) {
		super(message);
	}
}

function getApiInternal<T extends AzureExtensionApi>(
	azExts: AzureExtensionApiFactory[],
	extensionId: string,
	apiVersionRange: string,
	options: GetApiOptions,
): T {
	return <T>(
		callWithTelemetryAndErrorHandlingSync(
			...
			(context: IActionContext) => {
				...
				const matchedApiVersion: string | null = semver.maxSatisfying(
					apiVersions,
					apiVersionRange,
				);
				if (matchedApiVersion) {
					...
					const minApiVersion: string | null = semver.minSatisfying(
						apiVersions,
						"",
					);
					let message: string;
					...
					if (
						...
						message = vscode.l10n.t(
							'API version "{0}" for extension id "{1}" is no longer supported. Minimum version is "{2}".',
							apiVersionRange,
							extensionId,
							minApiVersion,
						);
