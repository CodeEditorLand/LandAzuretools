...
export class AzExtUserInput implements types.IAzureUserInput {
	...
	public async showOpenDialog(
		options: types.AzExtOpenDialogOptions,
	): Promise<Uri[]> {
		addStepTelemetry(
			this._context,
			options.stepName,
	...
	public async showWorkspaceFolderPick(
		options: types.AzExtWorkspaceFolderPickOptions,
	...
	public async showWarningMessage<T extends MessageItem>(
		...
		try {
			...
			const result = await showWarningMessage<T>(
				this._context,
				message,
				...args,
			);
...
export function addStepTelemetry(
	context: IInternalActionContext,
	...
	if (!stepName) {
		stepName = description
			? `${stepType}|${convertToStepName(description)}`
			: stepType;
	}
