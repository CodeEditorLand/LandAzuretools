...
export async function showQuickPick<
	TPick extends types.IAzureQuickPickItem<unknown>,
>(
	...
	try {
		...
		const result = await new Promise<TPick | TPick[]>(
			async (resolve, reject): Promise<void> => {
					...
					quickPick.onDidAccept(async () => {
						try {
							if (options.canPickMany) {
								...
								const selectedItem: TPick | undefined =
				...
				try {
					...
					if (options.canPickMany && options.isPickSelected) {
						...
						quickPick.selectedItems = quickPick.items.filter((p) =>
...
>(
	...
	if (options.canPickMany && options.placeHolder) {
		options.placeHolder += l10n.t(
			" (Press 'Space' to select and 'Enter' to confirm)",
		);
	}
...
async function getRecentlyUsedKey(
	options: types.IAzureQuickPickOptions,
...
export async function createQuickPickItems<
	TPick extends types.IAzureQuickPickItem<unknown>,
>(
	picks: TPick[] | Promise<TPick[]>,
	options: types.IAzureQuickPickOptions,
	groups: QuickPickGroup[],
	recentlyUsedKey: string | undefined,
	...
	picks = await bumpHighPriorityAndRecentlyUsed(
		picks,
		globalState,
		!!options.suppressPersistence,
		recentlyUsedKey,
	);

	if (picks.length === 0) {
		if (options.noPicksMessage) {
			picks.push(<TPick>(<unknown>{
				label: options.noPicksMessage,
		...
		if (options.canPickMany) {
			throw new Error(
				'Internal error: "canPickMany" and "enableGrouping" are not supported at the same time.',
			);
		}
...
async function bumpHighPriorityAndRecentlyUsed<
	T extends types.IAzureQuickPickItem<unknown>,
>(
	picks: T[],
	globalState: Memento,
	...
	const recentlyUsedValue: string | undefined =
	...
	if (recentlyUsedValue) {
		recentlyUsedIndex = await asyncFindIndex(
			picks,
			async (p) => (await getRecentlyUsedValue(p)) === recentlyUsedValue,
		);
		...
		if (recentlyUsedIndex >= 0) {
			...
			if (!recentlyUsedItem.suppressPersistence) {
				...
				if (!recentlyUsedItem.description) {
					...
					!recentlyUsedItem.description.includes(recentlyUsed)
				) {
...
function stableSortPicks<T extends types.IAzureQuickPickItem<unknown>>(
	picks: T[],
	recentlyUsedIndex: number,
): T[] {
	function getPriorityAsNumber(
		pick: types.IAzureQuickPickItem<unknown>,
		index: number,
	): number {
	...
	const sortableFacade: [index: number, priority: number][] = picks.map(
		(pick, index) => [index, getPriorityAsNumber(pick, index)],
	);
...
function getGroupedPicks<TPick extends types.IAzureQuickPickItem<unknown>>(
	groups: QuickPickGroup[],
): TPick[] {
...
async function getRecentlyUsedValue(
	item: types.IAzureQuickPickItem<unknown>,
): Promise<string> {
