...
import {
	GetOrgReposReqParams,
	getRepositoriesByOrg,
	getRepositoriesByUser,
	GetUserReposReqParams,
} from "../wrappers/getRepositories";
...
export class GitHubRepositoryListStep extends AzureWizardPromptStep<GitHubContext> {
	...
	private async getPicks(
		...
		this.picks.push(
			...(context.gitHubOrg
				? (await getRepositoriesByOrg(context, orgRepoParams)).map(
						(repo) => {
							return {
								label: repo.name,
								data: {
				...
				: (await getRepositoriesByUser(context, userRepoParams)).map(
						(repo) => {
							return {
								label: repo.name,
								data: {
