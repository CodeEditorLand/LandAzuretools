...
import { getActionBasedIconPath, Status } from "../utils/actionUtils";
import { gitHubUrlParse } from "../utils/gitHubUrlParse";
import { ActionWorkflowRuns } from "../wrappers/getActions";
import { getJobs, GetJobsParams, Job, Jobs } from "../wrappers/getJobs";
...
export class ActionItem implements TreeElementBase {
	...
	private get contextValue(): string {
		...
		if (
			<Status>nonNullProp(this.actionWorkflowRuns, "status") ===
			Status.Completed
		) {
	...
	async getChildren(): Promise<TreeElementBase[]> {
		...
		if (jobsData?.total_count) {
			return jobsData.jobs.map(
				(job: Job) =>
					new JobItem(this.id, this.extensionPrefixContextValue, job),
			);
		} else {
			return [];
		}
