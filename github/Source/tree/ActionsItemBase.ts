...
import {
	...
	getActions,
	GetActionsListWorkflowRunsParams,
} from "../wrappers/getActions";
...
export abstract class ActionsItemBase implements TreeElementBase {
	...
	async getChildren(): Promise<TreeElementBase[]> {
		const sourceControl: GitHubSourceControl | undefined =
			await this.getSourceControl();

		const actionsListWorkflowRuns: ActionsListWorkflowRuns | undefined =
		...
		if (actionsListWorkflowRuns?.total_count) {
			return actionsListWorkflowRuns.workflow_runs.map(
				(awr) =>
					new ActionItem(
						this.id,
						this.contextValueExtensionPrefix,
						awr,
					),
			);
